...
- Scope routing: personal->L1, organizational->L2, ecosystem->L3, protocol->L4
...
CHECKPOINT_DIR = Path(__file__).parent.parent.parent / '.resurrection_checkpoints'

def init_checkpoint_dir():
    """Ensure checkpoint directory exists."""
    CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)

def get_checkpoint_file(citizen: str) -> Path:
    """Get checkpoint file path for a citizen."""
    return CHECKPOINT_DIR / f'{citizen}_processed.txt'

def load_processed_files(citizen: str) -> set:
    """Load set of already-processed files for a citizen."""
    checkpoint_file = get_checkpoint_file(citizen)
...
def save_checkpoint(citizen: str, file_path: str):
...
def clear_checkpoints():
    """Clear all checkpoint files (for fresh start)."""
    if CHECKPOINT_DIR.exists():
        for f in CHECKPOINT_DIR.glob('*_processed.txt'):
            f.unlink()
        logger.info(f'Cleared checkpoints from {CHECKPOINT_DIR}')

def get_checkpoint_stats() -> dict:
    """Get checkpoint statistics for all citizens."""
    stats = {}
    for citizen in CITIZENS:
        processed = load_processed_files(citizen)
        stats[citizen] = len(processed)
    return stats
...
def sanitize_value(value: str) -> str:
    """
    AGGRESSIVE sanitization to remove ALL characters that break Cypher syntax.

    This sacrifices some semantic richness for reliability - better to have
    clean data than fail 95% of insertions.
    """
    ...
    # Remove unicode/emoji (causes Cypher syntax errors)
    value = value.encode('ascii', 'ignore').decode('ascii')

    # Remove ALL asterisks (markdown bold/italic - paired or orphaned)
    value = value.replace('*', '')

    # Remove ALL brackets and braces (subentity markers, lists, etc)
    value = value.replace('[', '').replace(']', '')
    value = value.replace('{', '').replace('}', '')

    # Remove ALL backticks (code blocks)
    value = value.replace('`', '')
    ...
    # Remove common narrative prefixes that leak into values
    narrative_patterns = [
        r'^(Okay|Ok|So|Well|Now|Then|And|But|However|Therefore)[,\s]+',
        r'^(I think|I feel|I believe|I wonder|I notice|I see)[,\s]+',
        r'^\s*\([^)]*\)\s*',  # Remove parenthetical asides
    ]
    ...
    # Remove problematic punctuation that breaks Cypher strings
    ...
    # Final cleanup: ensure no empty strings or just punctuation
    if not value or value.isspace() or all(c in '.,;:-' for c in value):
...
def parse_trace_formations(content: str) -> Tuple[List[Dict], List[Dict]]:
    ...
    for match in re.finditer(node_pattern, content, re.DOTALL | re.IGNORECASE):
        ...
        for line in block_content.split('\n'):
            ...
            # STOP at blank lines (end of property section)
            if not line_stripped:
                break

            # STOP at markdown separators or headings
            if line_stripped.startswith('---') or line_stripped.startswith('###'):
                break

            # Parse property lines only
...
def process_conversation_file(file_path: Path, citizen: str, r: redis.Redis, embedding_service, dry_run: bool = False) -> Tuple[int, int, int, int, int, int]:
    ...
    try:
        ...
        # Insert L1 (personal/citizen) to citizen graph
        citizen_graph = get_citizen_graph_name(citizen)
        n1_nodes_inserted = 0
        ...
        # Insert L2 (organizational)
        n2_nodes_inserted = 0
        for node in n2_nodes:
            if insert_node(r, ORG_GRAPH, node.copy(), embedding_service, dry_run):
                n2_nodes_inserted += 1

        n2_links_inserted = 0
        for link in n2_links:
            if insert_link(r, ORG_GRAPH, link.copy(), dry_run):
                n2_links_inserted += 1

        # Insert L3 (ecosystem)
        n3_nodes_inserted = 0
        for node in n3_nodes:
            if insert_node(r, ECOSYSTEM_GRAPH, node.copy(), embedding_service, dry_run):
                n3_nodes_inserted += 1

        n3_links_inserted = 0
        for link in n3_links:
            if insert_link(r, ECOSYSTEM_GRAPH, link.copy(), dry_run):
                n3_links_inserted += 1
...
def resurrect_all_roundrobin(citizens: List[str], r: redis.Redis, embedding_service, dry_run: bool = False) -> Dict[str, Dict]:
    ...
    init_checkpoint_dir()  # Ensure checkpoint directory exists
    for citizen in citizens:
        files = get_conversation_files(citizen)
        citizen_files[citizen] = files
        processed_files[citizen] = load_processed_files(citizen)
        logger.info(f"  {citizen.upper()}: {len(files)} files ({len(processed_files[citizen])} already processed)")

    # Create all graphs
    if not dry_run:
        for citizen in citizens:
            create_graph_if_not_exists(r, get_citizen_graph_name(citizen))
        create_graph_if_not_exists(r, ORG_GRAPH)
        create_graph_if_not_exists(r, ECOSYSTEM_GRAPH)
    ...
    for round_idx in range(max_files):
        for citizen in citizens:
            ...
            if round_idx < len(files):
                ...
                if str(file_path) in processed_files.get(citizen, set()):
                    continue
                ...
                save_checkpoint(citizen, str(file_path))
...
def main():
    ...
    parser.add_argument('--clear-checkpoints', action='store_true', help='Clear existing checkpoints for fresh start')
    ...
    if any(checkpoint_stats.values()):
        logger.info("Found existing checkpoints - will resume:")
        for c, count in checkpoint_stats.items():
            if count > 0:
                logger.info(f"  {c.upper()}: {count} files already processed")
    ...
    if args.delete_first and not args.dry_run:
        ...
        for citizen in CITIZENS:
            delete_graph(r, get_citizen_graph_name(citizen))
        delete_graph(r, ORG_GRAPH)
        delete_graph(r, ECOSYSTEM_GRAPH)
    ...
    logger.info(f"  L3 (ecosystem): {n3_nodes_total} nodes, {n3_links_total} links")
