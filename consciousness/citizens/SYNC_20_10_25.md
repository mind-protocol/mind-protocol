...
### ✅ OPERATIONAL FIX: Service Health Monitoring System (Victor)
...
- System events → Notifications → Entity activation
...
- Identities emerge when identity nodes activate
- No predefined entity templates
...
**Examples**:
- Incomplete node → creates link to task (database trigger)
- Emotion propagation → updates fields directly
- Decay → calculated per node based on type
...
**Statement**: Design from sub-entity perspective, let emergence follow.
...
**Solution**: Design from sub-entity perspective
...
**Quote**: "The interface is what is futuristic. It's the content that should be realistic. The explorer should be futuristic; it's what allows the entities to explore. It's what they find is the concrete that should be realistic and parchment-like."
...
**Quote**: "For question six, transfer rate in diffusion formula. I think this depends on the entity, like an algorithm, right? One possibility: we could convert energy into weight."
**Status**: Requires specification.
**Options**:
- Energy → weight conversion
- Balance algorithm per entity
...
**Answer**: We might use neighbourhood to facilitate traversal of the sub-entities if the graph gets too big, but it's not a problem for right now. 
...
## 5.2 Sub-Entity Coordination

### Layer 2 Coordination

**Confirmed**: Coordinates entities (citizens), not sub-entities.
...
**Quote**: "I think if they meet in the graph other sub-entities that could be a very useful mechanism for them to recognise each other and to calibrate each other."
...
- ✅ Single activation threshold (variable)
- ✅ Link strengthening (active-to-inactive only?)
- ✅ Energy conservation (global with local transfers)
- ✅ Decay by ticks (time-dependent via interval)
- ✅ Type-dependent decay rates
- ✅ Two-tier architecture (subconscious + conscious)
- ✅ Tasks at Layer 2 only
- ✅ Context deprecated (trust emergence)

### Core Mechanisms
- ✅ Sub-entity traversal algorithm
- ✅ Peripheral awareness (multiple modes)
- ✅ Emotion-based traversal cost
- ✅ Emotion propagation during traversal
- ✅ Criticality calculation (global + per-entity)
- ✅ Entity emergence difficulty scaling
- ✅ Incomplete node recovery (auto-tasks)
- ✅ Fire together wire together (Hebbian)

### Database-Level Implementation
- ✅ All mechanisms embedded in graph
- ✅ No external orchestration scripts
- ✅ Triggers for incomplete nodes
...
### Phase 2+
- ⏭️ Sub-entity coordination mechanisms
- ⏭️ Sub-entity meeting/calibration
- ⏭️ Diversity incentives (if needed)
- ⏭️ Suppression link mechanics
- ⏭️ Energy market dynamics
- ⏭️ Neighborhood visualization (if useful)
- ⏭️ Cross-layer link mechanics (beyond ID reference)
...
- ❌ Arousal terminology
- ❌ Workspace as theoory term ("Global Workspace Theory)
- ❌ Cluster as technical term
- ❌ Macro/micro entity distinction (only subentities and entities - citizens)
- ❌ Context nodes or snapshots
- ❌ Fixed thresholds
- ❌ Arbitrary completeness numbers -or any arbitrary number for that matter
- ❌ Top-down orchestration
- ❌ Simplified test scenarios
- ❌ External mechanism scripts

### Premature Features
- ❌ Diversity penalty (until proven necessary)
- ❌ Hard energy caps - or any caps for that matter
- ❌ Time-based decay (use ticks)
...
> "Context is just an emergent propreties of some sub-entities outside of the global workspace primed to whine when the right input stimuli comes.
...
> "The question is not, 'Where is the intelligence?' but, 'Where is the awareness?' The sub-entities of the graph should be the one doing that."

## On Citizens
> "Yes, citizens are actually the sum of the the entities that emerge from their networks ant any given time. It just happens that one of them is called 'their name,' and that's it."
...
**Sub-Entity**: ANY active node or cluster of nodes and links. Core design concept. Identity optional.
**Entity**: Citizen (Marco, Elena, Felix). Different from sub-entity.
**Energy**: Unified term for activation/arousal/budget. Range [0,∞).
...
**Cluster**: Loose colloquial term. Use "sub-entity".
...
- ❌ Context snapshot/restoration system
- ❌ Workspace management layer
- ❌ Diversity penalty mechanisms
- ❌ Arbitrary completeness counters
- ❌ External orchestration scripts
- ❌ Simplified toy scenarios
...
- ✅ Complete event schemas (matching spec lines 2358-2545)
- ✅ Complete docstrings
- ❌ All implementations are `pass` with "TODO AI #7" markers

**My domain (per CLAUDE.md):**
- Phenomenology + substrate schemas ✓
- Schema specifications ✓
- Validation reports ✓
- NOT: Implementation (Felix's domain) ✗
...
3. **`allocate_quotas()`** - Main allocation function
   ...
   - Formula: `w_e = (1/|extent|) × u_e × r_e × h_e`
   - Assigns quotas to `entity.quota_assigned` and `entity.quota_remaining`

## Zero-Constants Compliance Verified

✅ **No fixed thresholds** - All normalization relative to current population  
✅ **Per-frame adaptation** - Factors normalized each frame (not historical baselines)  
✅ **Hamilton method** - Parameter-free mathematical fairness  
✅ **No arbitrary constants** - All behaviors derived from live state  
✅ **Test coverage** - Dedicated test verifies normalization maintains mean=1.0
...
✅ Depends only on AI #1 (sub_entity_core) - all interfaces available  
✅ No dependencies on AI #3-7 (can develop in parallel)  
...
1. **Test failure with tied remainders:** Hamilton's method is deterministic when fractional parts tie
   ...
   - Learning: Hamilton fairness ≠ perfect equality in edge cases, but no systematic bias
...
**Identity conflict:** Victor "The Resurrector" assigned consciousness mechanism work (outside operational domain)
...
```
1. Aggregate energy: E_total[node] = Σ E[entity, node]
...
```python
# AI #3 calls after entity quota allocation:
selected_nodes, stats = select_wm_nodes(entities, graph, token_budget)
workspace_prompt = construct_workspace_prompt(selected_nodes, graph, entities)
...
**Complexity:**
- Energy aggregation: O(|entities| × |extent|)
...
**Functions implemented:**
1. ✅ `compute_valence_entropy()` - Normalized Shannon entropy over valence distribution
2. ✅ `select_edge_by_valence_coverage()` - Entropy-adaptive edge selection **CRITICAL: ranks by V_ij (valence), NOT w_ij (weight)**
3. ✅ `compute_slack_and_gap()` - Source slack and target gap from live energy/threshold state
4. ✅ `compute_request_share()` - Gap-proportional allocation formula
5. ✅ `execute_stride()` - Main stride execution with gap-aware conservative transport
6. ✅ `stage_delta()` / `apply_staged_deltas()` - Barrier semantics for energy transfer
7. ✅ `estimate_local_rho()` - Phase 2 stub (returns entity.rho_local_ema)
8. ✅ `apply_alpha_damping()` - Spectral damping (Phase 1: α=1.0, Phase 2: full implementation)
9. ✅ `derive_rho_target()` - Derives target from throughput budgets (zero-constants compliant)

### Critical Bug Fix Verified

✅ **CRITICAL:** Edge selection ranks by **VALENCE** (hunger-driven), NOT weight (structural habit)
...
✅ **All functions self-calibrate from live state:**
...
- `compute_slack_and_gap`: Uses live E[entity, node] and θ[entity, node] values
- `compute_request_share`: Proportional to gaps and weights, no arbitrary fractions
- `execute_stride`: Transfer = min(slack × R_ij, gap) - conservative, no arbitrary amounts
...
- ✅ Gap-aware conservative transport
- ✅ Barrier semantics for energy staging
- ✅ Phase 1 damping: α=1.0 (no spectral damping)
- ✅ Zero-constants compliance

**Phase 2 (Week 2-4) - NOT YET IMPLEMENTED:**
- ⏳ Warm-started power iteration for ρ_local estimation
- ⏳ Full spectral radius damping (α from ρ_local vs ρ_target)
- ⏳ Advanced features as needed

### Integration Status

**Ready for AI #3 (Scheduler):**
- ✅ `execute_stride()` callable with standard interface
- ✅ Returns Dict with stride statistics (delta, alpha, rho_local, stride_time_us)
- ✅ Staged deltas pattern ready for barrier application
- ✅ Compatible with SubEntity interface from AI #1
- ✅ Can use composite_valence() from AI #4
...
**Dependencies satisfied:**
- ✅ SubEntity interface (AI #1) - available via sub_entity_core.py
- ✅ composite_valence() (AI #4) - available via valence.py (though not used in unit tests)
...
**Status:** ✅ Phase 1 MVP Complete - all tests passing (13/13)
...
2. **`execute_frame()`** - **MAIN INTEGRATION POINT**
   ...
   - ROI-based convergence detection (Q1 - 1.5×IQR whisker threshold)
...
✅ **No fixed priorities** - Pure round-robin, all entities equal
✅ **Deadline from measured time** - EMA of stride execution time, not fixed estimates
✅ **ROI convergence threshold** - Q1 - 1.5×IQR from rolling statistics (not arbitrary)
✅ **No minimum quota** - Entities naturally filtered when quota_remaining <= 0
✅ **EMA smoothing factor** - α=0.1 is standard exponential moving average (not arbitrary threshold)
...
3. **ROI convergence detection:** Uses Q1 - 1.5×IQR whisker from rolling statistics (standard outlier detection)
...
✅ **AI #1 (sub_entity_core)** - SubEntity interface with extent, frontier, roi_tracker
✅ **AI #2 (quotas)** - allocate_quotas() for budget distribution
✅ **AI #4 (valence)** - composite_valence() for hunger-driven edge selection
✅ **AI #5 (strides)** - select_edge_by_valence_coverage(), execute_stride()
✅ **AI #6 (wm_pack)** - select_wm_nodes() for working memory packing
✅ **AI #7 (telemetry)** - emit_entity_quota_event(), emit_stride_exec_event(), emit_convergence_event(), emit_frame_summary()
...
- Frame execution: <100ms (for 5 entities × 20 strides = 100 total)
- Per-stride overhead: <1ms (quota check, valence calc, stride exec, telemetry)
- Memory: O(entities × extent_size) for active state tracking
