...
Entity membership (BELONGS_TO links) is not being created when nodes form.
...
3. **Membership weight calculation:**
   ...
   - Secondary entities: If node relevant to multiple entities, create multiple BELONGS_TO links with lower weights
...
Strategy:
1. Query all nodes without BELONGS_TO links
...
5. Create BELONGS_TO link with derived weight
...
✅ **BELONGS_TO links exist:** `MATCH ()-[:BELONGS_TO]->(:Subentity) RETURN count(*) > 0`
...
✅ **Backfill complete:** All existing nodes (>2000 across citizens) have at least one BELONGS_TO link
...
for citizen in ['felix', 'atlas', 'iris', 'luca', 'ada', 'victor']:
    ...
    result = g.query('MATCH ()-[:BELONGS_TO]->(e:Subentity) RETURN e.id, count(*) as members ORDER BY members DESC')
