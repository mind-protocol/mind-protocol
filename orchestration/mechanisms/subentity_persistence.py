...
class SubEntityPersistence:
    ...
    def normalize_memberships(self, graph: 'Graph') -> Dict[str, Dict[str, float]]:
        ...
        Returns:
            Dict[node_id, Dict[subentity_id, normalized_weight]]
        """
        ...
        for link in graph.get_links_by_type(LinkType.MEMBER_OF):
            ...
            subentity_id = link.target_id
            weight = link.weight

            if node_id not in normalized:
                normalized[node_id] = {}
            normalized[node_id][subentity_id] = weight
        ...
        for node_id, memberships in list(normalized.items()):
            ...
            if total > 1.0:
                ...
                for subentity_id in memberships:
                    memberships[subentity_id] /= total
    ...
    def upsert_subentities(self, graph: 'Graph') -> int:
        """
    ...
    def upsert_memberships(self, graph: 'Graph', normalized_weights: Dict[str, Dict[str, float]]) -> int:
        ...
        for node_id, memberships in normalized_weights.items():
            for subentity_id, weight in memberships.items():
                try:
                    # Use MERGE for idempotency
                    query = """
                    ...
                    logger.error(f"Failed to upsert membership {node_id}->{subentity_id}: {e}")
                    errors += 1
    ...
    def checkpoint(self, graph: 'Graph', force: bool = False, checkpoint_interval_s: float = 300.0) -> Dict[str, int]:
        ...
        try:
            ...
            stats['entities_upserted'] = self.upsert_subentities(graph)
...
def integrate_with_engine(engine):
    ...
    Returns:
        SubEntityPersistence instance for manual control if needed
    """
    persistence = SubEntityPersistence(engine.adapter, batch_size=500)

    # Store reference on engine
    engine.entity_persistence = persistence

    logger.info("[SubEntityPersistence] Integrated with consciousness engine")
    logger.info("[SubEntityPersistence] Checkpoints will occur during persist_to_database() calls")
