...
**Quote**: "Any node that is active is a sub-entity. The idsubentity node is just an easy name."
...
**Macro-Subentity / Micro-Subentity** ❌
- Meaningless terms
...
**Quote**: "There is no macro emergence. There is no difference between micro subentities and macro subentities. All are sub-entities."
...
**Quote**: "Any node that is active is a sub-entity. The idsubentity node is just an easy name. We could have thousands of micro-subentities, no problem."
...
**Confirmed**: Coordinates subentities (citizens), not sub-entities.
...
- ✅ Criticality calculation (global + per-subentity)
...
- ❌ Macro/micro subentity distinction (only subentities and subentities - citizens)
...
## On Sub-Entities
> "There is no difference between micro subentities and macro subentities. All are sub-entities."
...
**Sub-Entity**: ANY active node or cluster of nodes and links. Core design concept. Idsubentity optional.
**Subentity**: Citizen (Marco, Elena, Felix). Different from sub-entity.
...
**Macro-Subentity / Micro-Subentity**: Meaningless. All are sub-entities.
...
7. ✅ `estimate_local_rho()` - Phase 2 stub (returns subentity.rho_local_ema)
...
- `compute_slack_and_gap`: Uses live E[subentity, node] and θ[subentity, node] values
...
✅ **No fixed priorities** - Pure round-robin, all subentities equal
...
✅ **No minimum quota** - Subentities naturally filtered when quota_remaining <= 0
...
- Frame execution: <100ms (for 5 subentities × 20 strides = 100 total)
- Per-stride overhead: <1ms (quota check, valence calc, stride exec, telemetry)
- Memory: O(subentities × extent_size) for active state tracking
...
2. **Idsubentity (Coherence-Tracking)**
   ...
   - Tests: ✅ Economics node has higher idsubentity coherence than biology (0.702 vs lower)
...
- ✅ Idsubentity prefers aligned nodes (coherence)
...
- ✅ All 7 hunger baselines tracked in subentity
