...
**BELONGS_TO link exists:** AI_Agent(Luca in L2) → Subentity(Luca in L1 graph)

**Wake signal strength:**
```
signal = Σ (E_task × w_BELONGS_TO)
       = 3.95 × 0.85 (Task→Luca)
       = 3.36 (strong signal)
```
...
- BELONGS_TO(Project: Phase 3): w += 0.12 (80% share)
- BELONGS_TO(Concern: Validation): w += 0.12
- BELONGS_TO(Team: Pipeline): w += 0.12
...
**From REQUIRES links, traverse to BELONGS_TO:**
- Backend debugging → BELONGS_TO(Victor, w=0.9) OR BELONGS_TO(Atlas, w=0.85)
- Frontend implementation → BELONGS_TO(Iris, w=0.92)
- WebSocket protocols → BELONGS_TO(Victor, w=0.88)
...
- BELONGS_TO weights learned from past work
...
Energy through BELONGS_TO paths:
...
- REQUIRES(capability: frontend) → BELONGS_TO(Iris)
...
- BELONGS_TO weights strengthen for both on telemetry tasks
...
**Scenario:** Task with no clear owner, weak BELONGS_TO
...
**Problem: Weak BELONGS_TO weights**
- Task "Partnership technical overview": E = 3.5 (moderate urgency)
- BELONGS_TO(Luca): w = 0.35 (weak - some relevance to substrate expertise)
- BELONGS_TO(Ada): w = 0.28 (weak - some relevance to architecture)
- BELONGS_TO(Nicolas): w = 0.42 (weak - business development potentially relevant)
...
**Multiple BELONGS_TO paths strengthen slightly:**
...
- BELONGS_TO weights learn this pattern
...
Third time: Automatic, strong BELONGS_TO, direct citizen wake
...
- BELONGS_TO(Victor, w=0.88) - operational issues his domain
...
**Diffusion from Task explores other BELONGS_TO:**
- BELONGS_TO(Atlas, w=0.75) - infrastructure engineer, database expert
- BELONGS_TO(Felix, w=0.68) - core consciousness implementation
...
- BELONGS_TO(Victor) on "memory issues": w 0.88 → 0.81 (decreased)
- Hypothesis "GC fix for memory": w decreases globally

**Weight updates from success:**

Atlas's approach marked very useful:
- BELONGS_TO(Atlas) on "memory issues": w 0.75 → 0.84 (increased)
...
- Task B BELONGS_TO(Ada): Signal 4.8 × 0.92 = **4.42** (strong)
...
- Task C BELONGS_TO(Felix): Signal 4.5 × 0.89 = **4.01** (strong)
...
- Task D BELONGS_TO(Iris): Signal 4.2 × 0.91 = **3.82** (strong)
...
- BELONGS_TO for each citizen on domain work: w increases
...
- Example 1: Direct response via BELONGS_TO + threshold
...
2. **BELONGS_TO weights** (learned from co-activation + TRACE)
...
- Who wakes? → Energy × weight through BELONGS_TO
...
2. **Learned expertise** - BELONGS_TO weights strengthen through co-activation
