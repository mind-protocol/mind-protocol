...
```python
energy: float  # 0.0-1.0, current activation energy
...
```python
energy_transfer_coefficient: float  # For links - re-activation potential (0.0-1.0)
...
Tracks how retrieved memories re-activate energy state.
...
class EnergyFeedbackData(BaseModel):
    ...
    Enables energy feedback loop: retrieving high-energy memories
    increases current energy state.
    """
    ...
    recommended_energy_adjustment: float = Field(
        description="Suggested delta to current energy state (-0.3 to +0.3)"
    )

    energy_source_breakdown: Dict[str, float] = Field(
        description="Which memory types contributed most to energy transfer (by node_type)"
    )
```
...
# Energy decay rates by resolution state:
# - resolved: 0.2/day (fast decay)
# - in_progress: 0.05/day (slow decay)
# - blocked: -0.1/day (energy INCREASES)
```
...
class TraversalPath(BaseModel):
    ...
    path_energy_profile: List[float] = Field(
        description="Energy levels along the path"
    )
...
class ActivationTiers(BaseModel):
    ...
    focus: List[str] = Field(
        max_items=7,
        description="High-energy, immediately relevant nodes (working memory limit)"
    )

    peripheral: List[str] = Field(
        max_items=20,
        description="Medium-energy, contextually relevant nodes (available for activation)"
    )

    background: List[str] = Field(
        description="Low-energy but retrieved nodes (dormant context)"
    )

    # Tier assignment criteria
    focus_threshold: float = 0.7  # energy >= 0.7
    peripheral_threshold: float = 0.4  # 0.4 <= energy < 0.7
    # background: energy < 0.4
```
