...
### 4. Arousal Propagation
**Query:**
```cypher
MATCH (n)
WHERE n.last_mechanism_id = 'arousal_propagation'
  AND n.arousal_level > 0
RETURN count(n) as nodes_with_arousal,
       avg(n.arousal_level) as avg_arousal
```
**Expected:** Multiple nodes with arousal > 0
...
[Tick 1] Arousal propagation: Z nodes propagated
...
- **Ripple effects** → Arousal propagation (activation spreading)
