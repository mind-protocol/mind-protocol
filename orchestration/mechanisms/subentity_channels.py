...
class SubEntityChannelSelector:
    """
    ...
    def __init__(
        ...
        # Per-subentity observations: subentity_id -> deque
        self.observations: Dict[str, deque] = defaultdict(
            lambda: deque(maxlen=window_size)
        )

        # Subentity embeddings cache (subentity_id -> embedding)
        self.entity_embeddings: Dict[str, np.ndarray] = {}

        logger.info(
            f"[SubEntityChannelSelector] Initialized "
            f"(window_size={window_size}, min_samples={min_samples})"
        )

    def set_entity_embedding(self, subentity_id: str, embedding: np.ndarray):
        """
        ...
        """
        self.entity_embeddings[subentity_id] = embedding
        logger.debug(f"[SubEntityChannelSelector] Updated embedding for {subentity_id}")
    ...
    def add_observation(
        ...
        self.observations[subentity_id].append(obs)

        logger.debug(
            f"[SubEntityChannelSelector] Recorded {subentity_id}: "
    ...
    def _compute_affinity(
        ...
        Returns:
            Dict mapping subentity_id -> affinity score
        """
        affinities = {}
        ...
        for subentity_id in entity_ids:
            if subentity_id not in self.entity_embeddings:
                # No embedding - neutral affinity
                affinities[subentity_id] = 0.0
                continue

            # Compute cosine similarity
            entity_emb = self.entity_embeddings[subentity_id]
            entity_norm = entity_emb / (np.linalg.norm(entity_emb) + 1e-8)

            affinity = float(np.dot(stim_norm, entity_norm))
            affinities[subentity_id] = affinity
    ...
    def _compute_recent_success(
        ...
        Returns:
            Dict mapping subentity_id -> success score
        """
        ...
        for subentity_id in entity_ids:
            obs_list = self.observations.get(subentity_id, deque())

            for obs in obs_list:
                entity_flips[subentity_id] += obs.num_flips
                entity_gaps[subentity_id] += obs.gap_closed
        ...
        for subentity_id in entity_ids:
            if total_flips == 0 or total_gap_closed == 0:
                ...
                success_scores[subentity_id] = 0.0
                continue

            flip_share = entity_flips[subentity_id] / total_flips
            gap_share = entity_gaps[subentity_id] / total_gap_closed

            # Geometric mean
            success = np.sqrt(flip_share * gap_share)
            success_scores[subentity_id] = float(success)
    ...
    def _rank_z_normalize(
        ...
        Args:
            scores: Dict of subentity_id -> score

        Returns:
            Dict of subentity_id -> z-score
        """
        if len(scores) < 2:
    ...
    def _softmax(self, scores: Dict[str, float]) -> Dict[str, float]:
        ...
        Returns:
            Dict of subentity_id -> proportion (sum = 1.0)
        """
        if not scores:
            return {}
    ...
    def select_channels(
        ...
        Returns:
            Dict mapping subentity_id -> budget proportion π_e
        """
        ...
        if not any(
            ...
            logger.debug(
                f"[SubEntityChannelSelector] Bootstrap mode - equal split"
            )
        ...
        logger.debug(
            f"[SubEntityChannelSelector] Channel selection: "
    ...
    def get_stats(self) -> dict:
        ...
        for subentity_id, obs_list in self.observations.items():
            if len(obs_list) > 0:
                ...
                stats[subentity_id] = {
                    ...
                    "has_embedding": subentity_id in self.entity_embeddings
                }
