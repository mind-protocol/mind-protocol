...
Implement branching ratio (σ) measurement to derive global_arousal from network propagation dynamics. This enables self-organized criticality (σ ≈ 1.0) detection and multi-scale formula integration.
...
- Use global_arousal in formulas
...
class BranchingRatioTracker:
    ...
    Maps σ → global_arousal for multi-scale criticality.
    """
    ...
    def map_sigma_to_arousal(self, sigma: float) -> float:
        ...
        Mapping:
        - σ < 0.5:   global_arousal = 0.1        (dying, dormant)
        - σ 0.5-0.8: global_arousal = 0.2-0.4    (subcritical)
        - σ 0.8-1.2: global_arousal = 0.4-0.7    (critical, healthy)
        - σ > 1.2:   global_arousal = 0.7-1.0    (supercritical)
        ...
        """
        if sigma < 0.5:
            # Dying - very low arousal
            return 0.1

        elif sigma < 0.8:
            # Subcritical - low to moderate arousal
            # Linear interpolation: 0.5 → 0.2, 0.8 → 0.4
            return 0.2 + (sigma - 0.5) * 0.6

        elif sigma < 1.2:
            # Critical regime (healthy) - moderate to high arousal
            # Linear interpolation: 0.8 → 0.4, 1.2 → 0.7
            return 0.4 + (sigma - 0.8) * 0.75

        else:
            # Supercritical - high arousal, capped at 1.0
...
def consciousness_tick(self):
    ...
    # 5. Run mechanisms with global_arousal available
    global_arousal = consciousness_state["global_arousal"]
    self.run_scheduled_mechanisms(global_arousal)
...
def store_consciousness_state(self, consciousness_state: Dict[str, Any]):
    ...
    # SET state.global_arousal = $global_arousal,
...
def log_criticality(self, consciousness_state: Dict[str, Any]):
    ...
    print(f"[Cycle {self.tick_count}] "
          ...
          f"global_arousal={consciousness_state['global_arousal']:.3f} "
...
def test_branching_ratio_critical():
    ...
    assert 0.4 <= result['global_arousal'] <= 0.7
...
def test_branching_ratio_subcritical():
    ...
    assert result['global_arousal'] == 0.1
...
def test_branching_ratio_supercritical():
    ...
    assert result['global_arousal'] >= 0.7
...
def test_consciousness_engine_criticality():
    ...
    for i in range(10):
        ...
        assert 0.0 <= state['global_arousal'] <= 1.0
...
✅ **BranchingRatioTracker class created** - measures σ, maps to global_arousal
...
✅ **Global arousal available** - for multi-scale formula integration
...
1. Add `entity.arousal` field to SubEntity schema
