...
**Principle:** *Indirect coordination by default.* Agents coordinate through the graph (signals, tasks, evidence, learned weights). Direct messages are allowed but must route through a “Courier” subentity that logs everything to L2.
...
* **Expected yield (y):** predicted awareness recruitment: use φ/flow features from subentity boundary + link traces (z‑scored vs weekly baseline).
...
* **Affinity:** cosine(intent, citizen profile embeddings + their active subentities).
...
* L1 subentity layer lights up; WM will include the mission and top evidence.
...
* Update: link weights, node weights, citizen competence, source‑type yields (used by gates), subentity boundary ease/dominance if cross‑subentity strides occurred.
...
* **One canonical idsubentity thread per citizen** (for continuity, memory).
...
* **Direct chat allowed** only inside **Task Rooms** (short‑lived), proxied by a **Courier** subentity:
...
## 9) Are L2 “sub‑subentities” the same machinery? Yes.

Exactly as we unified neighborhoods↔subentities at L1, we run the **subentity layer at L2**:

* Functional subentities (Ops, DevEx, Partnerships) and semantic subentities (Authentication, CI, Billing) are L2 subentities.
* Stimuli feed those subentities; working memory at L2 is **5–7 active L2 subentities**; between‑subentity strides at L2 *produce intents*.
...
* **Scalable attention:** L2 subentities (chunks) make WM and traversal tractable; autonomy focuses on the 5–7 most meaningful org topics right now.
