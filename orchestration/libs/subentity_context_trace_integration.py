"""
SubEntity Context Integration for TRACE Capture - Priority 4 Implementation

Provides subentity context derivation and membership querying for context-aware
weight learning.
...
class SubEntityContextManager:
    """
    Manages subentity context derivation for TRACE reinforcement.
    ...
    def __init__(self, graph_store):
        ...
        self.last_wm_subentities = []  # Cache from last WM emit
        self.last_update_time = None

    def derive_entity_context(
        self,
        wm_subentities: Optional[List[str]] = None,
        trace_annotations: Optional[List[str]] = None,
        graph_name: str = "citizen_felix"
    ) -> List[str]:
        """
        Derive subentity context for TRACE reinforcement.

        Priority order (per spec Section D):
        1. wm_subentities (from wm.emit.selected_entities)
        2. trace_annotations (explicit [entity: X] marks in TRACE)
        3. Dominant entity (highest energy/threshold ratio)

        Args:
            wm_subentities: Entity IDs from working memory selection
            trace_annotations: SubEntity IDs from explicit TRACE annotations
            graph_name: Graph to query for dominant entity

Returns:
            List of subentity IDs to attribute TRACE to
        """
        # Priority 1: WM selected entities
        if wm_subentities and len(wm_subentities) > 0:
            logger.debug(f"[SubEntityContext] Using WM entities: {wm_subentities}")
            self.last_wm_subentities = wm_subentities
            self.last_update_time = datetime.now()
            return wm_subentities

        # Priority 2: Explicit TRACE annotations
        if trace_annotations and len(trace_annotations) > 0:
            logger.debug(f"[SubEntityContext] Using TRACE annotations: {trace_annotations}")
            return trace_annotations

        # Priority 3: Fallback to dominant entity
        dominant = self._get_dominant_entity(graph_name)
        if dominant:
            logger.debug(f"[SubEntityContext] Using dominant entity: {dominant}")
            return [dominant]

        # No context available - use cached WM entities if recent
        if self.last_wm_subentities and self.last_update_time:
            age = (datetime.now() - self.last_update_time).total_seconds()
            if age < 60.0:  # Within last minute
                logger.debug(f"[SubEntityContext] Using cached WM entities: {self.last_wm_subentities}")
                return self.last_wm_subentities

        # Absolute fallback: empty context (global learning only)
        logger.warning("[SubEntityContext] No entity context available - using global learning only")
        return []
...
Returns:
        ...
        try:
            ...
            if result and len(result) > 0:
                row = result[0]
                subentity_id = row[0]  # subentity_id
                ratio = row[3]  # ratio

                if ratio > 1.0:  # Only return if actually active
                    logger.debug(f"[SubEntityContext] Dominant entity: {subentity_id} (ratio={ratio:.2f})")
                    return subentity_id

        except Exception as e:
            logger.warning(f"[SubEntityContext] Failed to query dominant entity: {e}")
    ...
    def update_from_wm_event(self, wm_event: Dict[str, Any]):
        ...
        if wm_event.get('kind') == 'wm.emit':
            ...
            if selected_entities:
                self.last_wm_subentities = selected_entities
                self.last_update_time = datetime.now()
                logger.debug(f"[SubEntityContext] Updated from WM event: {selected_entities}")
...
class MembershipQueryHelper:
    ...
    def get_node_memberships(
        ...
        Args:
            ...
            subentity_ids: Entity IDs to filter by
            graph_name: Graph to query

Returns:
            Dict[node_id, Dict[subentity_id, membership_weight]]
        """
        if not node_ids or not subentity_ids:
            return {}
        ...
        try:
            ...
            UNWIND $subentity_ids AS subentity_id
            MATCH (n {name: node_id})-[r:MEMBER_OF]->(e:Subentity {id: subentity_id})
            RETURN n.name as node_id,
                   e.id as subentity_id,
                   r.weight as weight
            """
            ...
            if result and len(result) > 0:
                for row in result:
                    ...
                    if node_id not in memberships:
                        memberships[node_id] = {}
                    memberships[node_id][subentity_id] = weight
...
Returns:
            Dict[link_id, Dict[subentity_id, membership_weight]]
        """
        if not link_ids or not subentity_ids:
            return {}
        ...
        try:
            ...
            OPTIONAL MATCH (src)-[src_mem:MEMBER_OF]->(e:Subentity {id: subentity_id})
                ...
                for row in result:
                    ...
                    if link_weight > 0.0:
                        if link_id not in memberships:
                            memberships[link_id] = {}
                        memberships[link_id][subentity_id] = link_weight
...
# 1. Derive subentity context
subentity_context = self.subentity_context_manager.derive_subentity_context(
    wm_subentities=None,  # TODO: Get from last WM emit
