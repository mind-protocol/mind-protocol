...
class TraceParser:
    ...
    def _extract_entity_activations(self, content: str) -> Dict[str, str]:
        """Extract subentity activation levels: [entity_name: strong]"""
        activations = {}

        for match in self.entity_activation_pattern.finditer(content):
            subentity = match.group('subentity')
            level = match.group('level')

            # Keep highest activation level for each subentity
            if subentity not in activations or self._is_higher_activation(level, activations[subentity]):
                activations[subentity] = level

        logger.debug(f"[TraceParser] Found {len(activations)} subentity activations")
        return activations
    ...
    def _determine_primary_entity(self, activations: Dict[str, str]) -> str:
        """Determine primary (most activated) subentity."""
        if not activations:
            return None
...
"""
    ...
    for subentity, level in result.entity_activations.items():
        print(f"  - {subentity}: {level}")
