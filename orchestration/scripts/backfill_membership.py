...
Assigns primary_entity and BELONGS_TO links for all nodes missing membership.
...
4. Create BELONGS_TO link with weight=1.0, role='primary'
...
def get_unattributed_nodes(graph_name: str, r: redis.Redis) -> List[Dict[str, Any]]:
    """
    Get all nodes without primary_entity attribution.
    ...
    query = """
    MATCH (n:Node)
    WHERE n.primary_entity IS NULL OR n.primary_entity = ''
...
def get_default_entity_id(graph_name: str) -> str:
    """
    Get default entity ID for a citizen graph.
    ...
    Returns:
        Full entity ID: 'entity_citizen_felix_translator'
    """
    # Extract citizen name from graph_name
    citizen_id = graph_name.replace('citizen_', '')
    return f'entity_citizen_{citizen_id}_translator'
...
def assign_membership(
    ...
    """
    Assign node to entity (set property + create link).

    Args:
        graph_name: Graph to operate on
        node_name: Node.name to assign
        entity_id: Full entity ID (e.g., 'entity_citizen_felix_translator')
        r: Redis connection
    ...
    """
    if dry_run:
        logger.info(f"  [DRY RUN] Would assign {node_name} → {entity_id}")
        return True
    ...
    try:
        result = r.execute_command(
            ...
            query,
            '--params',
            f'{{"node_name": "{node_name}", "entity_id": "{entity_id}"}}'
        )
...
def backfill_graph(graph_name: str, r: redis.Redis, dry_run: bool = False) -> int:
    ...
    default_entity_id = get_default_entity_id(graph_name)
    logger.info(f"Default entity: {default_entity_id}")

    # Verify entity exists
    entity_check_query = f"MATCH (e:Subentity {{id: '{default_entity_id}'}}) RETURN e.id"
    entity_result = r.execute_command('GRAPH.QUERY', graph_name, entity_check_query)
    if len(entity_result) <= 1 or len(entity_result[1]) == 0:
        logger.error(f"  ✗ Entity {default_entity_id} does not exist in {graph_name}")
        logger.error(f"  ✗ Run entity bootstrap first!")
        return 0
    ...
    for i, node in enumerate(nodes, 1):
        ...
        if assign_membership(graph_name, node_name, default_entity_id, r, dry_run):
            success_count += 1

    logger.info(f"✅ Assigned {success_count}/{len(nodes)} nodes to {default_entity_id}")
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("ENTITY MEMBERSHIP BACKFILL")
    logger.info("=" * 80)
