...
- Remove energy from schema ✅
- Verify weight modulates traversal cost ✅
- Confirm no energy references in code ✅
...
- ✅ `substrate/schemas/energy_flow_mechanics.md` - Energy removed, energy-only architecture
...
- ⚠️ `substrate/schemas/valence_driven_exploration.md` - May need energy removal
- ⚠️ Other substrate docs - Search/replace energy references

**Implementation Files (Felix's domain):**
- ⏳ `substrate/schemas/consciousness_schema.py` - Remove energy field
...
**What Was Discovered:** Consciousness requires operating at criticality (phase transition between order and chaos). System needs BOTH global energy (system-wide) AND per-entity energy (individual activation) working together through bidirectional coupling.
...
2. **Per-Entity Energy** (Individual Entity Activation)
   - Range: 0.0 (inactive) to 1.0 (maximally active)
   - Each entity has distinct energy profile
   - Varies within global constraints
   - Reflects this specific entity's energy distribution

**How Global Energy is Computed:**

Global energy is **derived from branching ratio (σ)** - the emergent propagation dynamics of the network, NOT from entity energy aggregation.
...
propagated_energy = (
    base_energy
    * link_strength
    * entity_energy_multiplier  # Per-entity scale
...
entity_boost = 1.0 + (entity.energy * 0.3)
...
4. **Energy transfer** - memories re-activate energy → long-range correlations
...
**2. Global vs Entity Energy Coupling (Critical)**

**What to Track:**
- Global energy: System-wide consciousness level (0.0-1.0)
- Per-entity energy distribution: All entities across all nodes
...
```
Global Energy:   ───▲─▲─▼─▲───  (system-wide line)
Entity Energy:   Multiple lines per entity
  - Translator:   ─▲─▲───▼─▲──
  - Validator:    ──▼─▲─▲─▼───
  - Builder:      ─▲───▼─▲─▲──
```
...
propagated_energy = (
    ...
    * entity_multiplier     # (entity.energy + 1) / 2
...
│ Global Energy:  0.65 (healthy: 0.3-0.7) ━━━━━●─── │
...
- ✅ No energy on relations (clean architecture)
...
### Branching Ratio → Global Energy Mapping
...
def map_sigma_to_energy(sigma):
    ...
    σ 0.8-1.2: global_energy = 0.4-0.7    # Critical (healthy target)
    σ > 1.2:   global_energy = 0.7-1.0    # Supercritical (runaway)
    """
    if sigma < 0.5:
        return 0.1
    elif sigma < 0.8:
...
✅ **BranchingRatioTracker class works** - measures σ, maps to global_energy, rolling average stable
✅ **Engine integration complete** - measures after each cycle, stores state
✅ **Tests pass** - all 3 scenarios verified
✅ **ConsciousnessState queryable** - other systems can read global_energy
...
- Add entity.energy field to SubEntity
...
**Principle Applied:** Measure Emergent Properties - global energy derived from propagation dynamics, not entity aggregation.
