---
title: Subentity Weight Learning (BELONGS_TO)
status: stable
owner: @felix
...
summary: >
  Learn soft memberships (BELONGS_TO.weight) from co-activation of nodes with
...
# Subentity Weight Learning (BELONGS_TO)

## 1. Context — What problem are we solving?

We need memberships that reflect **which nodes truly belong** to each subentity so that:
- entity activation (read-out) is meaningful and stable,
- two-scale traversal has coherent within-entity neighborhoods,
...
- **Co-activation signal**: Node active (above \(\Theta_i\)) while entity active (above \(\Theta_E\)) within a frame/window → update EMA.  
- **WM presence**: entity selected for WM with node among its top members → positive evidence.  
- **TRACE reinforcement / formation quality**: when available, include as low-frequency but high-value signals (rank-z within entity-type cohorts).  
...
- **Novelty gate:** prefer evidence when the node’s membership set is under-diversified to avoid “entity creep”.  
...
For ranking members inside an entity, standardize log-weights by entity-type baselines (μ/σ); storage stays as plain `log_weight`. This keeps comparisons stable without mutating stored values. :contentReference[oaicite:67]{index=67}

## 3. Why this makes sense

### 3.1 Phenomenology

Belonging emerges from **repeated co-activation** while the entity is active and relevant to WM/TRACE—mirrors how humans form tight associations to a topic or role. :contentReference[oaicite:68]{index=68}
...
- Entities sharpen around **core** members (high standardized membership).  
...
- **Vs per-entity energy buffers:** conflates activation with belonging; duplicates energy state; violates substrate rules. :contentReference[oaicite:72]{index=72}
...
- Membership sparsity & **coherence EMA** vs entity size. :contentReference[oaicite:75]{index=75}

## 7. Failure Modes & Guards

- **Entity creep:** set minimum deltas + scheduled sparsification trims. :contentReference[oaicite:76]{index=76}  
- **Collapsed variance (all members equal):** alert on near-zero σ for a given entity-type. :contentReference[oaicite:77]{index=77}  
...
- **Runtime:** `mechanisms/sub_entity_traversal.py` (when entity flips / WM emission, update co-activation & WM signals). :contentReference[oaicite:79]{index=79}  
...
- For active entities, top members stabilize (low churn) while tail remains plastic.  
...
- When to **merge/split** entities based on membership distributions (bimodality tests)? :contentReference[oaicite:83]{index=83}  
- Learn entity-specific **decay** of membership weights (currently uniform schedule).
