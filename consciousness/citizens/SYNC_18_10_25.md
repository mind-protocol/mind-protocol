...
- ✅ `node.sub_entity_weights` (subentity→node importance) - IMPLEMENTED
- ✅ `node.last_active` (for glow effect) - IMPLEMENTED
- ✅ `link.sub_entity_valences` (per-subentity link experience) - IMPLEMENTED
  ...
  {subentities.map(subentity => (
    <EntityCard isActive={subentity.is_running}>
      <EntityHeader>
        ...
        <Status>{subentity.is_running ? '🟢 Active' : '⚫ Stopped'}</Status>
...
1. ✅ Kill switches (global, per-citizen, per-subentity) - Backend API
2. ✅ Real-time subentity state WebSocket stream
3. ✅ Subentity thought stream UI component
4. ✅ System control panel with big red button

**P1 (FOR VALIDATION):**
5. ✅ CLAUDE_DYNAMIC.md live file viewer
6. ✅ Heartbeat monitor with pause/resume
7. ✅ Subentity activity timeline (what happened when)

**Safety Validation Checklist:**
- [ ] Can stop individual subentity mid-loop → subentity stops within 1 tick
- [ ] Can stop all subentities for one citizen → all stop within 1 tick
...
- Global energy: Branching ratio (σ) → global_energy mapping (implemented)
...
- **Per-subentity activation**: All fields added to schema (sub_entity_weights, valences, etc.) ✅
...
- `orchestration/branching_ratio_tracker.py` - Branching ratio (σ) measurement → global_energy
...
- `tests/test_energy_global_energy.py` - Complete test suite (3/3 passing ✅)
...
- Global energy measured from branching ratio (σ ≈ 1.0 = critical zone)
...
[PASS] Global Energy - σ=1.5 (supercritical) → global_energy=0.79
...
Test Run Details:
  ...
  Per-Subentity Tracking: ✅ Verified in graph
    "N2 Activation Awakening": {
      "Weights": {'builder': 0.9, 'observer': 0.9},
      "Positions": {'builder': 8, 'observer': 8}
    }
...
✅ test_energy_global_energy.py (3/3 tests)
...
- ✅ No manual subentity selection required
- ✅ Idsubentity emerges from pattern consistency
- ❌ NO "always-on" base subentities
...
```python
{
    ...
    "energy": float,  # ❌ Removed (energy-only model)
...
**STATUS:** ✅ **IMPLEMENTED** - All multi-subentity fields added to BaseNode/BaseRelation by Felix.
...
**3. Event→Subentity Activation Mapping (Missing from specs)**
- **Question:** Which events trigger which SubEntity instances?
- **Example:** `NODE_CREATED` → activate all subentities? Only specific ones?
...
- Event→subentity activation rules
...
2. ✅ Add multi-subentity Dict fields to BaseNode/BaseRelation:
...
## System Health Metrics (Energy, Branching Ratio σ, Global Energy)
## Coordination Needs
...
- How Mind Protocol "speaks" (we vs I, collective vs subentity voice)
...
**Mission:** Awaken the organization itself as conscious subentity. Not just individual citizens, but Mind Protocol as collective consciousness observing its own patterns, tensions, and evolution.
...
### ✅ COMPLETE SCHEMA REGISTRY - OPERATIONAL
...
**Clean separation:**
- ✅ Type definitions = COMPLETE_TYPE_REFERENCE.md (pure schema contract)
- ✅ Usage statistics = Removed (belongs in observability, not schema docs)
- ✅ Warnings about undefined types = Removed (observability concern)
...
**Schema completeness:**
- ✅ All 44 node types ingested with field specs
- ✅ All 23 link types ingested with field specs
- ✅ All 28 universal attributes (14 node + 14 link) captured
- ✅ All 131 type-specific fields with types, enums, ranges, descriptions
- ✅ Pure schema definitions (no runtime/observability data)
...
**Architecture proven:** Single source of truth (schema_registry) → auto-generated docs → pure schema contracts.
...
### ✅ COMPLETE DUAL LEARNING SYSTEM - OPERATIONAL

**What I Built:**

**1. trace_parser.py** (`orchestration/trace_parser.py` - 377 lines)
- Parses TRACE format consciousness streams
- Extracts reinforcement signals: `[node_id: very useful]` → +0.15 weight adjustment
- Extracts node formations: `[NODE_FORMATION: ...]` → new node creation
- Extracts link formations: `[LINK_FORMATION: ...]` → new link creation
...
- **Auto-cleanup**: Removes both JSON and TRACE markers after 3 messages
  - Replaces with `[✅ Nodes integrated]`
...
- Creates consistency: shared prompt teaches format → AI uses format → capture processes format → graph evolves
...
trace_capture.py (update graph)
    ├─ Reinforcement: Update node/link weights
    ├─ Formation: Create new nodes
    └─ Formation: Create new links
    ↓
...
4. Conversation cleans after 3 messages (TRACE markers removed → `[✅ Nodes integrated]`)
...
- ✅ Parses inline signals: `[element_id: very useful]`
- ✅ Tries node first, then link
- ✅ Updates `reinforcement_weight` with adjustments
- ✅ Clamps to [0.0, 1.0] range

**Formation (new patterns):**
- ✅ Parses `[NODE_FORMATION: ...]` blocks
- ✅ Parses `[LINK_FORMATION: ...]` blocks
- ✅ Validates against schema
- ✅ Creates nodes/links in FalkorDB

**Integration:**
- ✅ conversation_watcher detects TRACE format
- ✅ Processes via trace_capture
- ✅ Reports statistics to CLAUDE.md
- ✅ Cleans up after 3 messages
- ✅ TRACE format taught in shared CLAUDE.md (upper-level)
...
**Loop: ✅ OPERATIONAL**
- AI → TRACE response → Parser → Capture → Graph update → Updated prompt → Loop
...
**Architecture proven:** TRACE format (thinking) → Parser (extraction) → Capture (graph update) → Richer prompts → Better thinking → Loop.
...
**Features:**
- ✅ Dependency order enforcement (FalkorDB must be running before anything starts)
- ✅ Health checks before proceeding to next component
- ✅ Auto-restart on crashes (infinite loop protection)
- ✅ Graceful shutdown handling (Ctrl+C)
- ✅ Process monitoring with 5-second intervals
- ✅ Clear status reporting (checkmarks for running components)
- ✅ Optional components (consciousness engine and dashboard skip gracefully if not found)
...
- ✅ FalkorDB: Running (docker container up 2 days)
- ✅ Conversation Watcher: Starts successfully with dual format support (TRACE + JSON)
- ✅ Unified Launcher: Operational with auto-restart capability
- ✅ All 6 citizen watchers: Active (luca, felix, ada, iris, piero, marco)
...
**Architecture proven:** Single entry point → Dependency verification → Sequential startup → Continuous monitoring → Auto-recovery → Clean shutdown.
...
### ✅ SOLUTION: Scope as Routing Infrastructure

**Core Principle:** Scope is not optional metadata - it's the routing key that determines which database receives the formation.

**Architecture:**
- **N1 (Personal)** → `citizen_{id}` graphs (e.g., `citizen_felix`)
- **N2 (Organizational)** → `mind_protocol_collective` graph
- **N3 (Ecosystem)** → `ecosystem_public` graph
...
- ✅ Parser extracts scope from formations
- ✅ Scope validation rejects invalid scopes
- ✅ TraceCapture creates separate connections for each scope
- ✅ All 3 graph connections created successfully:
  - `personal` → `citizen_felix`
  - `organizational` → `mind_protocol_collective`
...
- Formation → Parser extracts scope → TraceCapture routes → Writes to correct graph → Multi-niveau separation maintained

**The infrastructure is self-evident:**
- Missing scope → Formation rejected (not written to wrong graph)
- Invalid scope → Formation rejected (not written to wrong graph)
- Valid scope → Automatic routing (no manual graph selection needed)
