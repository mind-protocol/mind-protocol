...
**Note:** This document uses implementation terminology. For taxonomy and conceptual model, see `TAXONOMY_RECONCILIATION.md` and `glossary.md`. In clean taxonomy: **SubEntity** = Scale A (neighborhoods), **Mode** = Scale B (IFS meta-roles).
...
1. **SubEntities are neighborhoods** (functional roles like *Translator* or semantic topics like *consciousness_architecture*).
   They are **first‑class graph nodes** with members (other nodes) and **dynamic activation** derived from their members. This is **Scale A** of the two-scale architecture.
...
7. **Traversal is two‑scale** (Scale A architecture):
...
* **SubEntities** (Scale A) are neighborhoods (functional or semantic). They are **nodes that group nodes** via **MEMBER_OF** with weights. A SubEntity's activation (E_{\text{SubEntity}}) is **derived** from member nodes.
...
### 2.3 SubEntity (neighborhood - Scale A)
...
2b) **Multi‑scale traversal (two buckets)** - Scale A architecture
...
5. **Learning**: boundary precedence, link updates, node weight updates, membership adaptation (Hebbian on MEMBER_OF).
...
* [ ] Add **SubEntity** nodes + **MEMBER_OF** + **RELATES_TO**; compute SubEntity energy/threshold each frame.
...
* Think **single (E) per node** (fast), **weights** (slow), **SubEntities** as dynamic neighborhoods (Scale A).
...
* See `TAXONOMY_RECONCILIATION.md` for clean terminology: SubEntity (Scale A), Mode (Scale B), deprecated Entity.
