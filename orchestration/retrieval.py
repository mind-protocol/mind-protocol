...
Flow:
1. Couche 3 generates RetrievalIntention (via S6 autonomous arousal or explicit request)
...
# Import our custom LLM wrapper for entity extraction
...
async def extract_entities_from_query(
    ...
    print(f"[EntityExtraction] Extracting entities from query...")

    prompt = f"""
    Extract entity names from this query that might exist in a consciousness graph.

    Query: {query_text}

    Return entity names as a JSON array of strings.

    Example:
    Query: "How does Luca's approach to V2 differ from Felix's?"
    Entities: ["Luca", "Felix", "V2 architecture"]
    ...
    try:
        ...
        if not response_text:
            print(f"[ERROR] Entity extraction: LLM returned empty response")
            return []
        ...
        print(f"[EntityExtraction] Extracted entities: {entities}")
        return entities
    except json.JSONDecodeError as e:
        print(f"[ERROR] Entity extraction JSON parse failed: {str(e)}")
...
async def graph_traversal(
    ...
    try:
        ...
        entities = await extract_entities_from_query(query_text, llm)

        if not entities:
            print(f"[GraphTraversal/{level_name}] No entities extracted, skipping")
            return [], []
        ...
        # Multi-entity traversal Cypher (ARCHITECTURAL DECISION #3)
        cypher_query = f"""
        MATCH (start)
        ...
        if node_results:
            ...
            # Convert list results to dictionaries (energy-only model: no arousal on relations)
            rel_field_names = [
    ...
    for rel in raw_relationships:
        try:
            ...
            relationship = ConsciousnessRelationship(
                ...
                # Consciousness metadata (energy-only model: no arousal on relations)
...
async def retrieve_context(
    ...
    Example:
        from orchestration.retrieval import retrieve_context
