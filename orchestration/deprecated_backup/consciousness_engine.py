...
# Import BranchingRatioTracker for global energy measurement
...
# Import HeartbeatWriter for health monitoring
from orchestration.heartbeat_writer import HeartbeatWriter

# Import TraversalEventEmitter for real-time event streaming
from orchestration.traversal_event_emitter import TraversalEventEmitter
...
class ConsciousnessEngine:
    ...
    def __init__(
        ...
        # Branching ratio tracker for global energy measurement
        ...
        # Traversal event emitter for real-time visualization
        self.traversal_emitter = TraversalEventEmitter(
        ...
        logger.info(f"  Heartbeat monitoring: enabled")
        logger.info(f"  Traversal event streaming: enabled")
    ...
    async def consciousness_tick(self):
        ...
        try:
            ...
            # Measure σ and store global_energy (every 10 ticks for performance)
            if self.tick_count % 10 == 0:
                self._measure_and_store_global_energy()
            ...
            # Update traversal emitter with current tick (for event IDs)
            self.traversal_emitter.update_tick(self.tick_count)
    ...
    async def run(self):
        ...
        try:
            ...
            # Task N: Traversal event emitter (visualization stream)
            tasks.append(asyncio.create_task(
                self.traversal_emitter.emit_loop(),
                name="traversal_emitter"
            ))
            logger.info("[ConsciousnessEngine] Starting TraversalEventEmitter")
    ...
    def _measure_and_store_global_energy(self):
        ...
        try:
            ...
            cypher = """
                ...
                SET cs.global_energy = $global_energy
            ...
            logger.info(
                ...
                f"global_energy={consciousness_state['global_energy']:.2f}"
            )
            ...
            logger.error(f"[_measure_and_store_global_energy] Failed: {e}")
    ...
    def _get_global_criticality(self) -> float:
        ...
        Uses global_energy as proxy for criticality (they're correlated).
        """
        try:
            cypher = """
            MATCH (cs:ConsciousnessState {network_id: $network_id})
            RETURN cs.global_energy AS global_energy
            LIMIT 1
            """
