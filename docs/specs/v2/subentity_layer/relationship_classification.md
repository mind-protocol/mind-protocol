...
**Type:** Subentity Dynamics
**Phase:** 3 (Core consciousness mechanisms)
**Status:** Specified
**Source:** Discussion #010 - Subentity Competition Model (Nicolas's direct contribution)
...
Classify relationships between subentities as "collaborators" or "rivals" using embedding-based similarity rather than hard-coded rules. Link types (positive vs negative) modulate energy flow based on relationship classification. Creates emergent subentity dynamics from semantic similarity.
...
- Don't hard-code which subentities compete vs collaborate
- Use subentity embeddings to measure semantic similarity
...
**Nicolas's design:**
> "Use embedding-based subentity relationship classification (collaborators vs rivals) with energy modulation by link type."
...
**Each subentity has an embedding vector:**
...
class Subentity:
    ...
    def generate_embedding(self):
        """
        Create embedding from subentity's activation pattern

        Options:
        1. Average embeddings of cluster nodes
        2. Average embeddings of active nodes over time window
        3. Learned embedding from subentity behavior
        """
...
def classify_entity_relationship(entity_a, entity_b, threshold=0.7):
    ...
    # Cosine similarity between subentity embeddings
    similarity = np.dot(entity_a.embedding, entity_b.embedding)

    if similarity > threshold:
        return "collaborator"  # Semantically similar subentities
    else:
        return "rival"  # Semantically different subentities
...
def get_relationship_strength(entity_a, entity_b):
    ...
    Returns: float [0, 1]
    - 1.0: Perfect alignment (identical subentities)
...
def diffuse_energy_with_entity_relationships(graph, entity_a, entity_b):
    """
    Diffuse energy between two subentities with relationship modulation
...
**Scenario 1: Similar subentities (e.g., Translator + Architect)**
...
**Scenario 2: Different subentities (e.g., Validator + Builder)**
...
**Multiple approaches for subentity embeddings:**
...
2. **Activity-weighted averaging** (better)
   ...
   - Captures current subentity focus

3. **Temporal embedding** (dynamic)
   - Subentity embedding changes based on recent activity
   - Collaborators this hour might be rivals next hour

```python
def generate_activity_weighted_embedding(subentity, time_window=3600):
    """
    Weight node embeddings by recent activation

    Nodes recently activated contribute more to subentity embedding
    """

    weighted_embeddings = []

    for node in subentity.cluster_nodes:
        # Get node embedding
        node_emb = get_node_embedding(node)

        # Weight by recent energy
        recent_energy = node.get_energy_in_window(time_window)
        weight = recent_energy / sum([n.get_energy_in_window(time_window) for n in subentity.cluster_nodes])
...
- **Mechanism 03** (Subentity Emergence) - subentities must exist before classification
- **Mechanism 04** (Global Workspace) - subentity competition affects workspace
...
**For Felix:**
- Subentity embeddings generated on subentity emergence
- Embeddings cached (recalculate periodically or on significant change)
- Relationship classification cached per subentity pair
- Modulation applied during energy diffusion (adds one multiply per transfer)

**Performance:**
- Embedding generation: O(cluster size) per subentity
...
1. ✅ Similar subentities (high embedding similarity) classified as collaborators
2. ✅ Different subentities (low embedding similarity) classified as rivals
...
**Next Steps:** Integrate with Mechanism 07 (Energy Diffusion) and subentity emergence system
