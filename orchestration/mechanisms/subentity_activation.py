"""
SubEntity Activation - Deriving subentity energy from single-node substrate.

ARCHITECTURAL PRINCIPLE: Single-energy per node, subentity energy is DERIVED.
...
Where:
    ...
    - m̃_iE = normalized membership weight from MEMBER_OF link
...
subentities READ from that substrate rather than maintaining per-subentity channels.
...
class SubEntityCohortTracker:
    """
    Tracks cohort statistics for dynamic subentity thresholds.

    Uses rolling window of "touched subentities" (subentities with energy > 0)
    ...
    def update(self, touched_subentities: List[float]):
        """
        Update cohort statistics from subentities touched this frame.

        Args:
            touched_subentities: List of energy values for subentities with E > 0
        """
        if not touched_subentities:
            return

        # Add to rolling window
        for energy in touched_subentities:
...
@dataclass
class SubEntityActivationMetrics:
    """
    Metrics from subentity activation computation.
...
class LifecycleTransition:
    """
    Record of subentity lifecycle state transition.
...
def normalize_membership_weights(
    ...
    """
    Normalize MEMBER_OF membership weights to sum to 1.0.
...
def compute_subentity_activation(
    ...
    for node, _ in members_with_weights:
        ...
        subentity_energy += m_tilde * effective_energy

    return subentity_energy
...
def compute_entity_threshold(
    ...
    Returns:
        SubEntity activation threshold
    """
    from orchestration.core.types import LinkType
...
def dissolve_entity(
    ...
    This removes the entity from the graph and deletes all MEMBER_OF links
    to its members. Members return to the atomic node pool.

    Args:
        graph: Graph containing the entity
        entity: SubEntity to dissolve

    Side effects:
        - Removes entity from graph.subentities
        - Deletes all MEMBER_OF links to this entity
        - Members become free-floating nodes

    Example:
        >>> dissolve_entity(graph, low_quality_entity)
        >>> # SubEntity removed, members available for other entities
    """
    from orchestration.core.types import LinkType

    # Remove all MEMBER_OF links (release members)
    member_of_links = [
...
def update_entity_activations(
    ...
    Example:
        >>> tracker = SubEntityCohortTracker()
    ...
    for entity in graph.subentities.values():
        ...
        # Write back entity_activations to member nodes (for frontend viz)
        # Frontend needs to know which entities activated which nodes
        from orchestration.core.types import LinkType
        import time
        ...
        if members_with_weights:
            # Normalize membership weights (same as compute_subentity_activation)
            normalized_weights = normalize_membership_weights(members_with_weights)

            # Write back to each member node
            for node, _ in members_with_weights:
                # Initialize entity_activations dict if missing
                if not hasattr(node, 'entity_activations') or node.entity_activations is None:
                    node.entity_activations = {}

                # Compute this node's contribution to entity energy
                E_i = node.E
                Theta_i = node.theta
                effective_energy = max(0.0, E_i - Theta_i)
                m_tilde = normalized_weights[node.id]
                contribution = m_tilde * effective_energy

                # Write back to node
                node.entity_activations[entity.id] = {
                    "energy": contribution,
        ...
        if enable_lifecycle:
            # All entities follow normal lifecycle (no special treatment)
        ...
        # Create metrics
        metrics = SubEntityActivationMetrics(
            subentity_id=entity.id,
    ...
    for entity in entities_to_dissolve:
        logger.warning(f"[Lifecycle] DISSOLVING entity: {entity.id}, "
...
def learn_relates_to_from_boundary_stride(
    ...
    Args:
        source_entity: SubEntity energy is flowing from
        target_entity: SubEntity energy is flowing to
    ...
    if existing_link:
        ...
        new_link = Link(
            ...
            subentity=source_entity.id,
            goal="SubEntity boundary relationship learned from energy flow",
            mindstate="Boundary stride detection",
            confidence=0.5,  # Initial confidence
            formation_trigger="traversal_discovery",
            weight=learning_rate * energy_flow if energy_flow > 0 else 0.0,
            energy=energy_flow,
            scope="organizational"
        )

        # Attach runtime references for convenience
        new_link.source = source_entity
        new_link.target = target_entity
