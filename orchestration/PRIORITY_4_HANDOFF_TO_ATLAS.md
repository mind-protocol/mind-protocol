...
- ✅ Created 10 test BELONGS_TO (Deprecated - now "MEMBER_OF") relationships for testing
...
**Problem:** Entity bootstrap creates BELONGS_TO (Deprecated - now "MEMBER_OF") links in memory but never persists them.
...
redis-cli GRAPH.QUERY citizen_felix "MATCH (n)-[r:BELONGS_TO (Deprecated - now "MEMBER_OF")]->(e:Subentity) RETURN count(r)"
# Should return > 0 (currently returns 0)
```

**Why critical:** Without BELONGS_TO (Deprecated - now "MEMBER_OF"), membership-weighted learning can't work.
...
**For Atlas's tasks:**
- ✅ BELONGS_TO (Deprecated - now "MEMBER_OF") relationships persist to database
