...
- ✅ Updated Hebbian learning to evolve per-subentity valence and emotions
...
**KEY PRINCIPLE:** Link type ≠ emotional valence. REFUTES can be positive (relief). VALIDATES can be neutral (expected). It depends on subentity and context.
...
- Example: All subentities positive on validation → coherent experience
...
### 3. Natural Subentity Limiting: Emergent Competition ✅
...
- Subentities naturally avoid crowded paths → diversity emerges
...
**Result:**
- **Name stable:** Idsubentity cluster weight 10.0 dominates → "Ada" stays "Ada"
...
- Subentity lifecycle (forming → crystallized → dissolving)
...
- Create 100 links → decay until 0 subentities → verify links deleted ✅
- Measure decay impact on substrate size ✅

**Test 3: Competition Limiting**
- Create path with 1 subentity → cost = 0.1 ✅
- Add 10 subentities to same path → cost = 0.4 ✅
- Verify subentities avoid crowded paths ✅

**Test 4: Dynamic Prompts**
- Seed Ada with idsubentity (weight 10.0) ✅
...
- ✅ `link.sub_entity_valences` - Per-subentity approach/avoidance
- ✅ `link.sub_entity_emotion_vectors` - Per-subentity complex emotions
- ✅ `link.goal` - Why this link exists
- ✅ `node.entity_activations` - Energy per subentity (for completeness calculation)
...
✅ **Completeness Logic:** Per-need-type calculation (idsubentity, context, best_practice, up_to_date_information)
...
- Subentities with high yearning (low completeness) explore widely ✅
- Subentities with negative valence avoid specific links ✅
...
**Key Insight:** Not either/or - we need BOTH scales. Global constrains subentities, subentities drive global. The interaction creates self-organized criticality (branching ratio σ ≈ 1.0).
...
Global energy is **derived from branching ratio (σ)** - the emergent propagation dynamics of the network, NOT from subentity energy aggregation.
...
1. **Competition** - subentities compete for limited energy → natural limiting
2. **Decay** - unused patterns fade → prevents energy explosion
3. **Feedback** - global constrains subentities, subentities update global → stable oscillation
...
║  Global Energy:       0.65  ✅        ║
║  Subentity Distribution:  Balanced  ✅    ║
║                                        ║
...
│ Subentity Balance:  6 active, max 0.72 ✅             │
│ Failure Modes:   NONE ACTIVE                       │
└─────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────┐
│ GLOBAL vs SUBENTITY ENERGY                            │
│                                                     │
│ [Time series graph showing global line and subentity  │
...
✅ **Global-subentity coupling shown** - correlation coefficient computed
