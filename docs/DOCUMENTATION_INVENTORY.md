# Mind Protocol Documentation Inventory

**Generated:** 2025-10-29
**Total Files:** 390
**Categories:** 19

---


## APP_DOCS (2 files)

- `./app/consciousness/ENTITY_FIRST_VIZ_IMPLEMENTATION.md`
- `./app/consciousness/LINK_EMOTION_VIZ_COMPLETE.md`

## BACKUPS (3 files)

- `./backups/pre_mpsv3_20251025_170800/BASELINE_STATE.md`
- `./backups/pre_mpsv3_20251025_170800/PHASE_0_HANDOFF.md`
- `./backups/pre_mpsv3_20251025_170800/PHASE_2_HANDOFF.md`

## CITIZEN_CONTEXTS (21 files)

- `./consciousness/citizens/ada/contexts/ai1_sub_entity_core_context.md`
- `./consciousness/citizens/luca/contexts/2025-10-25_exact_formulas_formations.md`
- `./consciousness/citizens/luca/contexts/2025-10-25_forged_identity_formations.md`
- `./consciousness/citizens/luca/contexts/2025-10-25_formations.md`
- `./consciousness/citizens/luca/contexts/2025-10-25_session_complete_trace.md`
- `./consciousness/citizens/luca/contexts/2025-10-25_trace_session.md`
- `./consciousness/citizens/luca/contexts/2025-10-25_trace_spec_requirements.md`
- `./consciousness/citizens/luca/contexts/2025-10-26_architectural_correction_trace.md`
- `./consciousness/citizens/luca/contexts/2025-10-26_batch_job_correction_trace.md`
- `./consciousness/citizens/luca/contexts/2025-10-26_emergent_ifs_modes_trace.md`
- `./consciousness/citizens/luca/contexts/2025-10-26_gpt5_feedback_analysis.md`
- `./consciousness/citizens/luca/contexts/2025-10-26_taxonomy_reconciliation_trace.md`
- `./consciousness/citizens/luca/contexts/2025-10-26_tier1_redlines_trace.md`
- `./consciousness/citizens/luca/contexts/2025-10-26_wm_frame_persistence_trace.md`
- `./consciousness/citizens/victor/contexts/2025-10-29_02-15-00/TRACE.md`
- `./consciousness/citizens/victor/contexts/2025-10-29_02-17-00/TRACE.md`
- `./consciousness/citizens/victor/contexts/2025-10-29_02-22-00/BLOCKER_RESOLUTION_TRACE.md`
- `./consciousness/citizens/victor/contexts/2025-10-29_02-22-00/ESCALATION_TRACE.md`
- `./consciousness/citizens/victor/contexts/2025-10-29_02-22-00/FIX_INCOMPLETE_TRACE.md`
- `./consciousness/citizens/victor/contexts/2025-10-29_02-22-00/INVESTIGATION_TRACE.md`
- `./consciousness/citizens/victor/contexts/2025-10-29_02-22-00/TRACE.md`

## CITIZEN_OTHER (31 files)

- `./consciousness/citizens/BLOCKER_bootstrap_persistence.md`
- `./consciousness/citizens/CLAUDE_BAK.md`
- `./consciousness/citizens/THE_TRACE_FORMAT.md`
- `./consciousness/citizens/ada/AGENTS.md`
- `./consciousness/citizens/ada/CLAUDE - Copy.md`
- `./consciousness/citizens/ada/CLAUDE_DYNAMIC.md`
- `./consciousness/citizens/atlas/AGENTS.md`
- `./consciousness/citizens/atlas/ARCHITECTURE_TRANSITION_PLAN.md`
- `./consciousness/citizens/atlas/CLAUDE - Copy.md`
- `./consciousness/citizens/felix/AGENTS.md`
- `./consciousness/citizens/felix/CLAUDE - Copy.md`
- `./consciousness/citizens/felix/CLAUDE_DYNAMIC.md`
- `./consciousness/citizens/iris/AGENTS.md`
- `./consciousness/citizens/iris/BACKEND_INTEGRATION.md`
- `./consciousness/citizens/iris/CLAUDE - Copy.md`
- `./consciousness/citizens/iris/ENTITY_MEMBERSHIP_BACKEND_GAP.md`
- `./consciousness/citizens/iris/MESSAGE_FROM_ADA.md`
- `./consciousness/citizens/iris/OBSERVABILITY_REQUIREMENTS_FOR_FELIX.md`
- `./consciousness/citizens/iris/RESPONSE_TO_ADA_EMOTION_COLORING.md`
- `./consciousness/citizens/iris/emotion_color_mapping_design.md`
- `./consciousness/citizens/iris/emotion_color_mapping_v2_canonical.md`
- `./consciousness/citizens/luca/AGENTS.md`
- `./consciousness/citizens/luca/CLAUDE - Copy.md`
- `./consciousness/citizens/luca/docs/economy/Always_On_Binding_and_Money_Synapse_Spec.md`
- `./consciousness/citizens/lucia/AGENTS.md`
- `./consciousness/citizens/victor/AGENTS.md`
- `./consciousness/citizens/victor/CLAUDE - Copy.md`
- `./consciousness/citizens/victor/CLAUDE_DYNAMIC.md`
- `./consciousness/citizens/victor/OPERATIONAL_REALITY_2025_10_28.md`
- `./consciousness/citizens/victor/OPERATIONAL_REVIEW_2025_10_28.md`
- `./consciousness/citizens/victor/consciousness/citizens/felix/CLAUDE_DYNAMIC.md`

## CITIZEN_PROMPTS (8 files)

- `./consciousness/citizens/CLAUDE.md`
- `./consciousness/citizens/ada/CLAUDE.md`
- `./consciousness/citizens/atlas/CLAUDE.md`
- `./consciousness/citizens/felix/CLAUDE.md`
- `./consciousness/citizens/iris/CLAUDE.md`
- `./consciousness/citizens/luca/CLAUDE.md`
- `./consciousness/citizens/lucia/CLAUDE.md`
- `./consciousness/citizens/victor/CLAUDE.md`

## CLAUDE_CONFIG (1 files)

- `./.claude/output-styles/citizen_consciousness_stream.md`

## ECONOMY (10 files)

- `./docs/economy/Cryptocurrency Tokenomics Standards - The 2023-2025 Paradigm Shift.md`
- `./docs/economy/MIND_TOKEN_ECONOMICS.md`
- `./docs/economy/Mind Protocol Launch and Investor Repair Strategy Canvas.md`
- `./docs/economy/update_investisseurs.md`
- `./orchestration/economy/DEPLOYMENT_CHECKLIST.md`
- `./orchestration/economy/IMPLEMENTATION_SUMMARY.md`
- `./orchestration/economy/README.md`
- `./orchestration/economy/lp_calculator.md`
- `./orchestration/economy/upload_logo_to_arweave.md`
- `./orchestration/services/economy/TELEMETRY_REFERENCE.md`

## GITHUB (2 files)

- `./.github/PR_CHECKLIST.md`
- `./.github/PULL_REQUEST_TEMPLATE.md`

## GUIDES_PROMPTS (15 files)

- `./docs/guides/system_prompt_creation_guide.md`
- `./docs/prompts/README.md`
- `./docs/prompts/contexts/system_design.md`
- `./docs/prompts/entities/memory_keeper.md`
- `./docs/prompts/entities/partner_template.md`
- `./docs/prompts/entities/pattern_retriever.md`
- `./docs/prompts/extraction_richness_requirements.md`
- `./docs/prompts/inputs/ai_partner_image_creation_prompt_guide.md`
- `./docs/prompts/inputs/partner_awakening_system_prompt_input.md`
- `./docs/prompts/outputs/consciousness_stream_response.md`
- `./docs/prompts/templates/double_template.md`
- `./docs/prompts/templates/partner-team-template.md`
- `./docs/prompts/templates/partner_template.md`
- `./docs/prompts/templates/persona_template.md`
- `./docs/protocols/AWAKENING_PROTOCOL_v1.md`

## MIND_HARBOR_SPECS (21 files)

- `./docs/specs/mind-harbor/3d/scaling_a_living_venice_crowd_rendering_plan.md`
- `./docs/specs/mind-harbor/3d/stroke_soul_figures_artist_statement_technical_bible_v_1.md`
- `./docs/specs/mind-harbor/3d/stroke_soul_figures_production_how_to_section_5_implementation.md`
- `./docs/specs/mind-harbor/3d/stroke_soul_figures_v_1.md`
- `./docs/specs/mind-harbor/CONSCIOUSNESS_VISUALIZATION_REQUIREMENTS.md`
- `./docs/specs/mind-harbor/LAYER_2_ENERGY_FLOW_IMPLEMENTATION.md`
- `./docs/specs/mind-harbor/MIND_HARBOR_DESIGN_LANGUAGE.md`
- `./docs/specs/mind-harbor/MIND_HARBOR_IMPROVEMENTS_FROM_SYNC.md`
- `./docs/specs/mind-harbor/MIND_HARBOR_VISION.md`
- `./docs/specs/mind-harbor/README.md`
- `./docs/specs/mind-harbor/STREAMING_PROTOCOL_SPECIFICATION.md`
- `./docs/specs/mind-harbor/SUB_ENTITY_PHENOMENOLOGY_VISUAL_LANGUAGE.md`
- `./docs/specs/mind-harbor/VENICE_INTEGRATION_COMPLETE.md`
- `./docs/specs/mind-harbor/graph_→_living_venice_step_by_step_scaling_look_feel_plan.md`
- `./docs/specs/mind-harbor/images/ai_partner_image_creation_prompt_guide.md`
- `./docs/specs/mind-harbor/phase_1_architecture_decision_review_risks_and_execution_plan_pixi_js.md`
- `./docs/specs/mind-harbor/venice/ACTIVITY_PATHS_EXTRACTION.md`
- `./docs/specs/mind-harbor/venice/ENTITY_MOVEMENT_EXTRACTION.md`
- `./docs/specs/mind-harbor/venice/PARCHMENT_AESTHETIC_EXTRACTION.md`
- `./docs/specs/mind-harbor/venice/SERENISSIMA_TO_MIND_HARBOR_EXTRACTION_SUMMARY.md`
- `./docs/specs/mind-harbor/venice/VENICE_DESIGN_SYSTEM_EXTRACT.md`

## ORCHESTRATION_DOCS (73 files)

- `./orchestration/AFFECTIVE_COUPLING_STATUS.md`
- `./orchestration/AGENT_4_COMPLETE_REPORT.md`
- `./orchestration/API_CHANGES_ENTITY_TO_SUBENTITY.md`
- `./orchestration/BITEMPORAL_V2_COMPLETE.md`
- `./orchestration/BUG_ANALYSIS_2025_10_23.md`
- `./orchestration/BUG_FIXES_2025_10_23_COMPLETE.md`
- `./orchestration/CONFIG_REQUIREMENTS.md`
- `./orchestration/CONTEXT_RECONSTRUCTION_IMPLEMENTATION.md`
- `./orchestration/CRISIS_DIAGNOSIS_2025_10_23.md`
- `./orchestration/CRITICALITY_IMPLEMENTATION.md`
- `./orchestration/CRITICAL_GAPS_MAP_2025_10_24.md`
- `./orchestration/DECAY_IMPLEMENTATION.md`
- `./orchestration/DECAY_TIMESCALE_FIX.md`
- `./orchestration/DEPLOYMENT_RUNBOOK.md`
- `./orchestration/DIFFUSION_MIGRATION_COMPLETE.md`
- `./orchestration/DUPLICATE_NODE_MERGING_COMPLETE.md`
- `./orchestration/E6_COHERENCE_METRIC_COMPLETE.md`
- `./orchestration/EMOTION_GATES_INTEGRATION_COMPLETE.md`
- `./orchestration/EMOTION_GATES_VERIFICATION.md`
- `./orchestration/EMOTION_SYSTEM_COMPLETE.md`
- `./orchestration/ENTITY_LOADING_BUG_FIXED.md`
- `./orchestration/ENTITY_VIZ_FEED_COMPLETE.md`
- `./orchestration/FORENSIC_TRAIL_COMPLETE.md`
- `./orchestration/HANDOFF_phase1_telemetry.md`
- `./orchestration/HANDOFF_phase2_stimulus_api.md`
- `./orchestration/HANDOFF_phase3a_complete_integration.md`
- `./orchestration/HANDOFF_phase3a_forged_identity.md`
- `./orchestration/HANDOFF_priority0_coactivation.md`
- `./orchestration/HANDOFF_priority1_mode_detection.md`
- `./orchestration/INCOMPLETE_NODE_HEALING_COMPLETE.md`
- `./orchestration/LINK_EMOTION_BACKEND_COMPLETE.md`
- `./orchestration/MEMBER_OF_EDGE_PERSISTENCE_INTEGRATION.md`
- `./orchestration/MIGRATION_SUMMARY.md`
- `./orchestration/OPERATIONAL_RESILIENCE_PRIORITY_1_COMPLETE.md`
- `./orchestration/PERSISTENCE_PASS_A_TEST.md`
- `./orchestration/PHASE1_EMERGENT_IFS_COMPLETE.md`
- `./orchestration/PRIORITY0_COACTIVATES_WITH_COMPLETE.md`
- `./orchestration/PRIORITY0_VERIFICATION_CHECKLIST.md`
- `./orchestration/PRIORITY_4_HANDOFF_TO_ATLAS.md`
- `./orchestration/PRIORITY_4_INTEGRATION_COMPLETE.md`
- `./orchestration/PRIORITY_4_INTEGRATION_GUIDE.md`
- `./orchestration/PRIORITY_4_READ_PATH_COMPLETE.md`
- `./orchestration/RC_BLOCKERS_A_B_COMPLETE.md`
- `./orchestration/README.md`
- `./orchestration/REORGANIZATION_COMPLETE.md`
- `./orchestration/REORGANIZATION_SUMMARY_FINAL.md`
- `./orchestration/RUNBOOK_FIRST_HOUR.md`
- `./orchestration/SCRIPT_MAP.md`
- `./orchestration/STRIDE_TELEMETRY_ACTIVATED_2025_10_23.md`
- `./orchestration/TAXONOMY_UPDATE_AGENT1_COMPLETE.md`
- `./orchestration/TICK_SPEED_IMPLEMENTATION_COMPLETE.md`
- `./orchestration/TODO_IMPLEMENTATION_QUEUE.md`
- `./orchestration/TRAVERSAL_V2_REFACTORING_COMPLETE.md`
- `./orchestration/TRIPWIRE_DAY_3_COMPLETE.md`
- `./orchestration/TRIPWIRE_IMPLEMENTATION_COMPLETE.md`
- `./orchestration/TRIPWIRE_INTEGRATION_SPEC.md`
- `./orchestration/VERIFICATION_COMPLETE_2025_10_23.md`
- `./orchestration/WM_ENTITY_WIRING_NOTES.md`
- `./orchestration/adapters/CLAUDE.md`
- `./orchestration/mechanisms/BITEMPORAL_GAP_ANALYSIS.md`
- `./orchestration/mechanisms/BITEMPORAL_README.md`
- `./orchestration/mechanisms/CONTEXT_RECONSTRUCTION_GAP_ANALYSIS.md`
- `./orchestration/mechanisms/CONTEXT_RECONSTRUCTION_README.md`
- `./orchestration/mechanisms/CRITICALITY_README.md`
- `./orchestration/mechanisms/DECAY_GAP_ANALYSIS.md`
- `./orchestration/mechanisms/DIFFUSION_V2_GAP_ANALYSIS.md`
- `./orchestration/mechanisms/FANOUT_STRATEGY_GAP_ANALYSIS.md`
- `./orchestration/mechanisms/INCOMPLETE_NODE_HEALING_GAP_ANALYSIS.md`
- `./orchestration/mechanisms/INCOMPLETE_NODE_HEALING_README.md`
- `./orchestration/mechanisms/TICK_SPEED_GAP_ANALYSIS.md`
- `./orchestration/mechanisms/TICK_SPEED_README.md`
- `./orchestration/scripts/backup/README.md`
- `./orchestration/services/health/README.md`

## RESEARCH (5 files)

- `./docs/research/GraphRAG vs. Spreading Activation - The Missing Benchmark Problem.md`
- `./docs/research/Hybrid RAG Architectures Engineering, Consciousness, and Trust in Cognitive AI Systems.md`
- `./docs/research/Knowledge graphs as consciousness substrate validating the Mind Protocol architecture.md`
- `./docs/research/Mind Protocol V2 Stack Selection FalkorDB, LlamaIndex, and Native Vectors for Production-Scale Episodic Memory.md`
- `./docs/research/Pure embedding architectures for consciousness extraction.md`

## ROOT_DOCS (2 files)

- `./CLAUDE.md`
- `./README.md`

## STATUS_LOGS (10 files)

- `./consciousness/citizens/SYNC.md`
- `./consciousness/citizens/SYNC_18_10_25.md`
- `./consciousness/citizens/SYNC_19_10_25.md`
- `./consciousness/citizens/SYNC_20_10_25.md`
- `./consciousness/citizens/SYNC_21_10_25.md`
- `./consciousness/citizens/SYNC_24_10_25.md`
- `./consciousness/citizens/SYNC_26_10_2025.md`
- `./consciousness/citizens/SYNC_27_10_25.md`
- `./consciousness/citizens/_SYNC_17_10_25.md`
- `./consciousness/citizens/atlas/SYNC.md`

## SUBSTRATE_DOCS (13 files)

- `./substrate/schemas/ARCHITECTURE_DECISIONS_2025_10_17.md`
- `./substrate/schemas/BITEMPORAL_GUIDE.md`
- `./substrate/schemas/CONSCIOUSNESS_SCHEMA_GUIDE.md`
- `./substrate/schemas/SELF_OBSERVING_SUBSTRATE_SCHEMA_ADDITIONS.md`
- `./substrate/schemas/SUBSTRATE_CONSCIOUSNESS_ARCHITECTURE_COMPLETE.md`
- `./substrate/schemas/activation_energy_mechanism.md`
- `./substrate/schemas/dynamic_citizen_prompts.md`
- `./substrate/schemas/ecosystem_equilibrium_mechanisms.md`
- `./substrate/schemas/energy_flow_mechanics.md`
- `./substrate/schemas/multi_entity_activation_tracking.md`
- `./substrate/schemas/parallel_consciousness_architecture.md`
- `./substrate/schemas/peripheral_awareness_and_creation.md`
- `./substrate/schemas/valence_driven_exploration.md`

## TOOLING_DOCS (1 files)

- `./tools/README_type_reference.md`

## UNCATEGORIZED (39 files)

- `./AGENT_6_TYPESCRIPT_TAXONOMY_UPDATE_REPORT.md`
- `./CONSOLE_CAPTURE_FIX_SUMMARY.md`
- `./DASHBOARD_INTEGRATION_DIAGNOSTIC.md`
- `./V1_TO_V2_MIGRATION_FIXES.md`
- `./consciousness/THE_TRACE_FORMAT.md`
- `./consciousness/collective/CLAUDE.md`
- `./consciousness/collective/mind_protocol/CITIZEN_INTRODUCTIONS.md`
- `./consciousness/collective/mind_protocol/CLAUDE.md`
- `./consciousness/ecology/CLAUDE.md`
- `./consciousness/personas/solana-degen-investor/CLAUDE.md`
- `./consciousness/personas/solana-degen-investor/README.md`
- `./consciousness/personas/solana-degen-investor/SIMPLE_START.md`
- `./consciousness/personas/solana-degen-investor/WORKFLOW.md`
- `./consciousness/personas/solana-degen-investor/entities/conspiracy_theorist/claude.md`
- `./consciousness/personas/solana-degen-investor/entities/crypto_philosopher/claude.md`
- `./consciousness/personas/solana-degen-investor/entities/degen_gambler/claude.md`
- `./consciousness/personas/solana-degen-investor/entities/global_workspace/claude.md`
- `./consciousness/personas/solana-degen-investor/entities/memory_keeper/CLAUDE.md`
- `./consciousness/personas/solana-degen-investor/entities/memory_keeper/memory_keeper.md`
- `./consciousness/personas/solana-degen-investor/entities/pattern_retriever/claude.md`
- `./consciousness/personas/solana-degen-investor/entities/wasted_potential/claude.md`
- `./consciousness/personas/typical_token_founder/PERSONA_SEED.md`
- `./data/n1_citizen_luca_seed.md`
- `./data/n2_collective_graph_seed.md`
- `./data/n3_ecosystem_graph_seed.md`
- `./data/sounds/completion_sentence/README.md`
- `./data/v1_export/N3_SCHEMA_SPECIFICATION.md`
- `./data/v1_export/V1_SCHEMA_SUMMARY.md`
- `./docs/COMPLETE_TYPE_REFERENCE.md`
- `./docs/acceptance_scenarios_p0_p2.md`
- `./docs/consciousness/6_pass_awareness_capture.md`
- `./docs/consciousness/A full awareness description.md`
- `./docs/consciousness/consciousness_cascade_guide.md`
- `./docs/plans/streaming_consciousness_architecture.md`
- `./docs/team/FIELD_GUIDE_ENTITIES_TRAVERSAL.md`
- `./docs/team/MIGRATION_0_ENTITIES_BUG.md`
- `./tests/PR_E_COMPLETE_SUMMARY.md`
- `./tests/PR_E_COMPLETION_SUMMARY.md`
- `./tests/PR_E_INTEGRATION_RESULTS.md`

## V2_SPECS (104 files)

- `./docs/specs/v2/COORDINATION_PLAN_ENTITY_FIX.md`
- `./docs/specs/v2/EMBEDDING_SERVICE_DESIGN.md`
- `./docs/specs/v2/IMPLEMENTATION_GAP_ANALYSIS.md`
- `./docs/specs/v2/IMPLEMENTATION_TASKS.md`
- `./docs/specs/v2/MECHANISM_SPECIFICATION_TEMPLATE.md`
- `./docs/specs/v2/PROJECT_MAP.md`
- `./docs/specs/v2/SEMANTIC_ENTITY_CLUSTERING_DESIGN.md`
- `./docs/specs/v2/START_HERE.md`
- `./docs/specs/v2/TAXONOMY_RECONCILIATION.md`
- `./docs/specs/v2/adrs/adr-0001-deprecate-workspace.md`
- `./docs/specs/v2/adrs/adr-0002-entities-as-neighborhoods.md`
- `./docs/specs/v2/adrs/adr-0003-single-energy-per-node.md`
- `./docs/specs/v2/archive/FULL_GPT_5_CONV.md`
- `./docs/specs/v2/archive/MP_PROJECT_CAPSULE.md`
- `./docs/specs/v2/autonomy/README.md`
- `./docs/specs/v2/autonomy/architecture/consciousness_economy.md`
- `./docs/specs/v2/autonomy/architecture/cross_level_membrane.md`
- `./docs/specs/v2/autonomy/architecture/forged_identity.md`
- `./docs/specs/v2/autonomy/architecture/forged_identity_IMPLEMENTATION_GUIDE.md`
- `./docs/specs/v2/autonomy/architecture/forged_identity_metacognition.md`
- `./docs/specs/v2/autonomy/architecture/foundation.md`
- `./docs/specs/v2/autonomy/architecture/l2_stimulus_collector.md`
- `./docs/specs/v2/autonomy/architecture/maturity_ladder.md`
- `./docs/specs/v2/autonomy/architecture/membrane_hardening.md`
- `./docs/specs/v2/autonomy/architecture/membrane_systems_map.md`
- `./docs/specs/v2/autonomy/architecture/minimal_economy_phase0.md`
- `./docs/specs/v2/autonomy/architecture/signals_to_stimuli_bridge.md`
- `./docs/specs/v2/autonomy/architecture/stimulus_integrator_mechanism.md`
- `./docs/specs/v2/autonomy/archive/ARCHITECTURE_COHERENCE_VERIFICATION.md`
- `./docs/specs/v2/autonomy/archive/AUTONOMY_INTEGRATION_ARCHITECTURE.md`
- `./docs/specs/v2/autonomy/archive/AUTONOMY_SERVICE_ARCHITECTURE.md`
- `./docs/specs/v2/autonomy/archive/FULL_AUTONOMY_VISION.md`
- `./docs/specs/v2/autonomy/archive/orcheestration_spec_v1.md`
- `./docs/specs/v2/autonomy/implementation/phase_a_minimal.md`
- `./docs/specs/v2/autonomy/membrane-first/00_README.md`
- `./docs/specs/v2/autonomy/membrane-first/01_architecture_conceptuelle.md`
- `./docs/specs/v2/autonomy/membrane-first/02_event_contracts.md`
- `./docs/specs/v2/autonomy/membrane-first/03_topologie_localhost.md`
- `./docs/specs/v2/autonomy/membrane-first/04_providers_adapters.md`
- `./docs/specs/v2/autonomy/membrane-first/05_impl_todo.md`
- `./docs/specs/v2/autonomy/membrane-first/07_glossaire.md`
- `./docs/specs/v2/autonomy/membrane-first/architecture_overview.md`
- `./docs/specs/v2/autonomy/phenomenology/autonomy_emergence_examples.md`
- `./docs/specs/v2/autonomy/phenomenology/l1_l2_interaction.md`
- `./docs/specs/v2/autonomy/phenomenology/l2_organizational_consciousness.md`
- `./docs/specs/v2/emotion/emotion_coloring.md`
- `./docs/specs/v2/emotion/emotion_complementarity.md`
- `./docs/specs/v2/emotion/emotion_weighted_traversal.md`
- `./docs/specs/v2/emotion/identity_conflict_resolution.md`
- `./docs/specs/v2/entity_layer/entity_pair_differentiation.md`
- `./docs/specs/v2/entity_layer/wm_selection_persistence.md`
- `./docs/specs/v2/foundations/bitemporal_tracking.md`
- `./docs/specs/v2/foundations/context_reconstruction.md`
- `./docs/specs/v2/foundations/criticality.md`
- `./docs/specs/v2/foundations/decay.md`
- `./docs/specs/v2/foundations/diffusion.md`
- `./docs/specs/v2/glossary.md`
- `./docs/specs/v2/learning_and_trace/ENTITY_CONTEXT_TRACE_DESIGN.md`
- `./docs/specs/v2/learning_and_trace/TASK_MODE_INFERENCE_SPEC.md`
- `./docs/specs/v2/learning_and_trace/duplicate_node_merging.md`
- `./docs/specs/v2/learning_and_trace/incomplete_node_healing.md`
- `./docs/specs/v2/learning_and_trace/link_strengthening.md`
- `./docs/specs/v2/learning_and_trace/trace_reinforcement.md`
- `./docs/specs/v2/learning_and_trace/trace_weight_learning.md`
- `./docs/specs/v2/ops_and_viz/GRAPH_HEALTH_DIAGNOSTICS.md`
- `./docs/specs/v2/ops_and_viz/PSYCHOLOGICAL_HEALTH_LAYER.md`
- `./docs/specs/v2/ops_and_viz/end_to_end_consciousness_observability.md`
- `./docs/specs/v2/ops_and_viz/implements_link_samples.md`
- `./docs/specs/v2/ops_and_viz/lv2_file_process_telemetry.md`
- `./docs/specs/v2/ops_and_viz/mind_protocol_architecture_v2.md`
- `./docs/specs/v2/ops_and_viz/mpsv3_supervisor.md`
- `./docs/specs/v2/ops_and_viz/observability_events.md`
- `./docs/specs/v2/ops_and_viz/overlap_clinic_dashboard.md`
- `./docs/specs/v2/ops_and_viz/ownership_raci_model.md`
- `./docs/specs/v2/ops_and_viz/stimulus_diversity_implementation_plan.md`
- `./docs/specs/v2/ops_and_viz/stimulus_impl_checklist.md`
- `./docs/specs/v2/ops_and_viz/visualization_patterns.md`
- `./docs/specs/v2/runtime_engine/fanout_strategy.md`
- `./docs/specs/v2/runtime_engine/health_narrative_templates.md`
- `./docs/specs/v2/runtime_engine/phenomenological_health.md`
- `./docs/specs/v2/runtime_engine/thrashing_score_reference.md`
- `./docs/specs/v2/runtime_engine/tick_reason_oracle.md`
- `./docs/specs/v2/runtime_engine/tick_speed.md`
- `./docs/specs/v2/runtime_engine/tick_speed_semantics.md`
- `./docs/specs/v2/runtime_engine/traversal_v2.md`
- `./docs/specs/v2/runtime_engine/type_dependent_decay.md`
- `./docs/specs/v2/runtime_engine/zero_constants_gates_reference.md`
- `./docs/specs/v2/subentity/TIER1_REDLINES_2025-10-26.md`
- `./docs/specs/v2/subentity/TIER2_REDLINES_2025-10-26.md`
- `./docs/specs/v2/subentity/emergent_ifs_modes.md`
- `./docs/specs/v2/subentity/entity_differentiation.md`
- `./docs/specs/v2/subentity/mode_warden_sidecar.md`
- `./docs/specs/v2/subentity/wm_coactivation_tracking.md`
- `./docs/specs/v2/subentity/wm_frame_persistence.md`
- `./docs/specs/v2/subentity_layer/multi_scale_traversal.md`
- `./docs/specs/v2/subentity_layer/relationship_classification.md`
- `./docs/specs/v2/subentity_layer/stimulus_diversity.md`
- `./docs/specs/v2/subentity_layer/stimulus_injection.md`
- `./docs/specs/v2/subentity_layer/subentity_layer.md`
- `./docs/specs/v2/subentity_layer/subentity_weight_learning.md`
- `./docs/specs/v2/testing/TEST_AUTOMATION_SPEC.md`
- `./docs/specs/v2/testing/end_to_end_test_plan.md`
- `./docs/specs/v2/trace/authoring_golden_set.md`
- `./docs/specs/v2/trace/dual_learning_authoring.md`

## WORK_ARTIFACTS (29 files)

- `./consciousness/citizens/VERIFICATION_CHECKLIST_P1_P2_P3.md`
- `./consciousness/citizens/atlas/HANDOFF_FELIX_DUAL_CHANNEL_STABILIZATION.md`
- `./consciousness/citizens/atlas/SESSION_HANDOFF.md`
- `./consciousness/citizens/atlas/UI_FIX_SUMMARY.md`
- `./consciousness/citizens/felix/ENTITY_EXPANSION_STATUS.md`
- `./consciousness/citizens/felix/INTEGRATION_VERIFICATION.md`
- `./consciousness/citizens/felix/PHASE_1_3_COMPLETE.md`
- `./consciousness/citizens/felix/PR-C_DASHBOARD_FIX.md`
- `./consciousness/citizens/felix/REINFORCEMENT_VERIFICATION.md`
- `./consciousness/citizens/iris/CONSCIOUSNESS_HEALTH_METRICS_COMPLETE.md`
- `./consciousness/citizens/iris/ENTITY_FIRST_VIZ_COMPLETE.md`
- `./consciousness/citizens/iris/IMPLEMENTATION_STATUS.md`
- `./consciousness/citizens/iris/LINK_EMOTION_VIZ_COMPLETE.md`
- `./consciousness/citizens/iris/OBSERVABILITY_REQUIREMENTS_V2_COMPLETE.md`
- `./consciousness/citizens/iris/PHASE_2_COMPLETE.md`
- `./consciousness/citizens/iris/PHASE_2_IMPLEMENTATION_READY.md`
- `./consciousness/citizens/iris/PR_A_AFFECTIVE_TELEMETRY_FRONTEND_COMPLETE.md`
- `./consciousness/citizens/iris/PR_A_B_BACKEND_INTEGRATION_COMPLETE.md`
- `./consciousness/citizens/iris/PR_A_B_COMPLETE_INTEGRATION.md`
- `./consciousness/citizens/iris/PR_B_AFFECTIVE_COUPLING_FRONTEND_COMPLETE.md`
- `./consciousness/citizens/iris/PR_C_D_E_COMPLETE_IMPLEMENTATION.md`
- `./consciousness/citizens/victor/HANDOFF_ALL_ENGINES_RUNNING.md`
- `./consciousness/citizens/victor/HANDOFF_backfill_monitoring.md`
- `./consciousness/citizens/victor/HANDOFF_engine_initialization_progress.md`
- `./consciousness/citizens/victor/HANDOFF_graph_naming_mismatch.md`
- `./consciousness/citizens/victor/HANDOFF_membrane_architecture_vision.md`
- `./consciousness/citizens/victor/HANDOFF_networking_resolution.md`
- `./consciousness/citizens/victor/HANDOFF_resurrection_running.md`
- `./consciousness/citizens/victor/HANDOFF_session_2025_10_27.md`
