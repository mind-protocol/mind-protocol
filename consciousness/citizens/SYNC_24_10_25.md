...
- **Found:** 3 files using different types (BELONGS_TO (Deprecated - now "MEMBER_OF") vs MEMBER_OF)
...
- **Acceptance:** `MATCH ()-[:BELONGS_TO (Deprecated - now "MEMBER_OF")]->(:Subentity)` > 0, drill-down shows members
...
**CRITICAL ERROR:**
```
AttributeError: type object 'LinkType' has no attribute 'BELONGS_TO (Deprecated - now "MEMBER_OF")'
File "diffusion_runtime.py", line 439: if link.link_type == LinkType.BELONGS_TO (Deprecated - now "MEMBER_OF"):
```

**ROOT CAUSE:** Updated LinkType enum to MEMBER_OF but didn't update all code references

**EMERGENCY FIX DEPLOYED:**
- ✅ Found 19 occurrences of `LinkType.BELONGS_TO (Deprecated - now "MEMBER_OF")` across 9 files
...
- ✅ LinkType enum: BELONGS_TO (Deprecated - now "MEMBER_OF") → MEMBER_OF
...
- ✅ No remaining BELONGS_TO (Deprecated - now "MEMBER_OF") references in code
...
- ✅ Manual Cypher confirms: `BELONGS_TO (Deprecated - now "MEMBER_OF")` links ARE created
...
2. Migrate `BELONGS_TO (Deprecated - now "MEMBER_OF")` → `MEMBER_OF` (semantic: nodes can have multiple entities)
...
**PLANNED REFACTOR: BELONGS_TO (Deprecated - now "MEMBER_OF") → MEMBER_OF**

**Rationale:** Nodes are MEMBERS OF (potentially multiple) Subentities, not owned by them

**Scope:**
1. **LinkType enum** (`orchestration/core/types.py`) - Add `MEMBER_OF`, deprecate `BELONGS_TO (Deprecated - now "MEMBER_OF")`
2. **Code references** (21 files, 79 occurrences) - Systematic rename
3. **Database migration** - Rename existing `BELONGS_TO (Deprecated - now "MEMBER_OF")` → `MEMBER_OF` relationships
...
- 18 other files referencing BELONGS_TO (Deprecated - now "MEMBER_OF")

**Database migration:**
```cypher
// Rename all BELONGS_TO (Deprecated - now "MEMBER_OF") → MEMBER_OF
MATCH ()-[r:BELONGS_TO (Deprecated - now "MEMBER_OF")]->()
...
1. **orchestration/core/types.py**
   - `BELONGS_TO (Deprecated - now "MEMBER_OF")` → `MEMBER_OF` in LinkType enum
...
4. **orchestration/scripts/backfill_membership.py** *(pending update)*
   - TODO: Update to use MEMBER_OF instead of BELONGS_TO (Deprecated - now "MEMBER_OF")

### Database Migration
- ✅ **citizen_felix**: 11 BELONGS_TO (Deprecated - now "MEMBER_OF") → MEMBER_OF
- ✅ **citizen_luca**: 264 BELONGS_TO (Deprecated - now "MEMBER_OF") → MEMBER_OF
...
**VERIFICATION:**
```bash
# Confirmed zero BELONGS_TO (Deprecated - now "MEMBER_OF") relationships remain
MATCH ()-[r:BELONGS_TO (Deprecated - now "MEMBER_OF")]->() RETURN count(r)  # → 0 for all graphs
