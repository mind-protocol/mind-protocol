...
MINIMUM_ENERGY_THRESHOLD: float = 0.001  # Remove entities below this threshold
...
def decay_node_energy(
    ...
    """
    Apply exponential decay to entity energy on node.

    Formula: e_new = e * exp(-decay_rate * duration)

    Args:
        node: Node to decay
        entity: Entity to decay energy for
...
def decay_tick(
    ...
    """
    Execute one tick of energy decay for entity across entire graph.
    ...
    for node in graph.nodes.values():
        energy_before = node.get_entity_energy(entity)
