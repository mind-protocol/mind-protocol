...
class SubEntity:
    ...
    async def _get_outgoing_links(self, node_id: str) -> List[Dict[str, Any]]:
        ...
        Returns list of dicts with: source_id, target_id, link_strength, target_energy
        Energy-first architecture - energy is deprecated.
        """
        try:
            cypher = """
            ...
            RETURN id(source) as source_id,
                   ...
                   target.energy as target_energy
            LIMIT 50
            """
            ...
            for row in result:
                links.append({
                    ...
                    'target_energy': float(row[3]) if row[3] is not None else 0.5
                })
    ...
    async def _calculate_link_score(self, link_data: Dict[str, Any], need: Need) -> float:
        ...
        Energy is deprecated. Scoring based on link strength and target importance.
        ...
        # Weighted sum (energy-first: no energy, no emotion for now)
