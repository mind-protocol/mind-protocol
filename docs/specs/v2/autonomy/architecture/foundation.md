...
* **Chunk‑first cognition.** Attention traverses **subentities** (neighborhoods/topics/modes), not millions of atomic links.
...
3. **Emergent, not commanded:** We don’t script “do X at 9am.” We **inject stimuli**; subentities light up; traversal and working‑memory assemble what needs doing. Autonomy is a side‑effect of a healthy substrate, not a cronjob.
...
5. **Multi‑scale truth:** Links carry the *narrative trace* (what has been traversed, with φ, precedence, recency), nodes carry *energy*, and **subentities** (neighborhoods) carry the *chunk‑level focus*. All three scales are needed for believable autonomy.
...
* **Why:** Keeps work **close to the mind** that will be most effective, and reuses the exact same activation machinery we already trust (stimulus → subentities → traversal).
...
### 3.7 Learning everywhere (links, nodes, subentities)

* **What:** Link φ/precedence/flow are updated by strides; node/log_weight reinforced by usefulness and traversal; subentity boundary ease/dominance learns from cross‑subentity flips.
...
* **What:** Every decision emits a timestamped, causally linked event. Iris subscribes, replays frames, and shows both **subentity‑scale beams** and **link‑scale particles**.
...
| WM shows 5–7 active subentities/topics, not 50 nodes        | Subentity layer + greedy WM knapsack by energy‑per‑token                                | Phenomenology matches “what I’m thinking about” |
...
* **Flat atomic traversal:** high branching and fragmented WM.
  → Subentity layer reduces branching 30–100× and gives WM coherent chunks.
...
* Fewer idsubentities/accounts to manage.
...
3. **Phase‑B:**
   ...
   * Subentity‑first WM and between‑subentity jumps integrated.
...
* It **aligns cognition** (subentities/WM/chunks) with **operations** (intents/priorities/verification).
