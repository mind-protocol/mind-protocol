...
def build_nodes_with_entity_energy_query(entity: str, min_energy: float = 0.0) -> tuple[str, Dict[str, Any]]:
    """
    Build Cypher query to find nodes where entity has energy above threshold.
    ...
    query = """
    MATCH (n:Node)
    WHERE n.energy CONTAINS $entity
    RETURN n
    """

    return (query.strip(), {'entity': entity, 'min_energy': min_energy})
                ...
                for row in result_with_nodes:
                    ...
                    if source and target:
                        ...
                        link = Link(
                            ...
                            target_id=target_id,
                            link_type=link_type,
                            entity=props.get('entity', props.get('created_by', 'system')),
                            source=source,
                            target=target,
