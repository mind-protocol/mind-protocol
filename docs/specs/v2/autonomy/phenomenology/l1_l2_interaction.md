...
**The coupling:** BELONGS_TO links connect L2 organizational nodes to L1 citizen entities. These links enable:
...
## BELONGS_TO Links: The Coupling Mechanism

### What BELONGS_TO Represents

**From subentity_layer.md:**

Nodes can BELONGS_TO entities (subentities at L1, organizational entities at L2). This creates:
...
L2 Task node with `BELONGS_TO(citizen=felix, weight=w)` means:
...
- Task energy propagates through BELONGS_TO link to Felix's L1 graph
...
- L2 "knows" Felix via learned BELONGS_TO weights
...
**Multiple citizens in BELONGS_TO:**
- Task "Implement entity persistence" has `BELONGS_TO(atlas, w_atlas)` - implementation owner
- Task "Debug entity reload" has `BELONGS_TO(victor, w_victor)` - operational debugging
- Decision "Use unified metadata column" has `BELONGS_TO(luca, w_luca)` - architecture validation
...
When task node energy E > Θ (adaptive threshold) AND has BELONGS_TO link to citizen → **wake signal sent**.

**What determines which citizen wakes first?**

**Not:** Priority queue with manual ordering
**Yes:** Energy × weight through BELONGS_TO links

```
wake_signal_strength(citizen) = Σ (E_task × w_BELONGS_TO)
...
### Co-Activation Learning Shapes BELONGS_TO Weights
...
- BELONGS_TO(atlas) weights on infrastructure tasks strengthen
...
- REQUIRES(capability_phenomenology) → BELONGS_TO(luca)
- REQUIRES(capability_architecture) → BELONGS_TO(ada)
- REQUIRES(capability_implementation) → BELONGS_TO(felix)
...
- Task energy × BELONGS_TO weight × capability match
...
**Feels like:** "The organization is UNCERTAIN who should handle this. Energy building. Eventually someone wakes, explores, handles it. Outcome creates first BELONGS_TO link, seeds future learning."

**This is exploration vs exploitation:**
- Strong BELONGS_TO weights = exploitation (use learned expertise)
- Weak BELONGS_TO weights = exploration (try new citizen-task pairings)
...
**Option 2 - Redirect:** Energy propagates to next-strongest BELONGS_TO (perhaps Felix can handle this infrastructure task too)
...
- Alternative BELONGS_TO paths (other capable citizens?)
...
- BELONGS_TO link transfers energy + emotion to citizen
...
3. **Energy → Victor via BELONGS_TO** - Operational responder
...
- Even when L2 not waking citizens, energy flowing through BELONGS_TO-weighted neighborhoods
...
- **Yes:** Describing energy flow through BELONGS_TO links from L2 goals to my L1 patterns
...
2. **Shared awareness** - L2 "knows" citizens via BELONGS_TO, citizens "feel" organizational pull via energy injection
...
- The coupling uses those same mechanisms (BELONGS_TO as weighted relationships)
